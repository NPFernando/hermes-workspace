...
| `src/components/update-center-notifier.tsx` | Added Naveen smart-update card + conflict resolution modal with AI analysis; moved release-notes storage to `src/lib/update-notes.ts` and removed the startup pendingReleaseNotes popup (now in NotificationHub digest) | Smart-update: personal. Digest refactor: PR candidate | Smart-update parts: keep always. Digest refactor: drop if upstream ships its own combined popup |
| `src/components/notification-hub.tsx` | Hub now owns the startup "what changed" decision: polls update status, folds agent release notes + workspace changelog into one `UpdateDigestModal` (was two independent popups) | Not yet submitted upstream — good PR candidate | Adopt upstream's version if they ship equivalent popup coordination |
| `src/components/update-digest-modal.tsx` | New: single combined update popup (agent commit sections + workspace changelog), marks both seen-keys on dismiss | Not yet submitted upstream — good PR candidate | Adopt upstream's version if merged |
| `src/components/whats-new-modal.tsx` | Exported `ChangeList` + added `markWhatsNewSeen()` for reuse by the digest modal; `WhatsNewModal` itself no longer mounted by the hub (kept for Settings) | Not yet submitted upstream | Small additive change; merge conflicts unlikely |
| `src/lib/update-notes.ts` | New: shared release-notes types + localStorage seen-state (`storeNotes`, `markNotesSeen`, `readStoredNotes`), extracted from update-center-notifier | Not yet submitted upstream — good PR candidate | Adopt upstream's version if merged |
| `src/stores/notification-center-store.ts` | New: persisted notification inbox (items + prefs: autoPopupDigest, toastOnNewItems, showUpdateCards) | Not yet submitted upstream — good PR candidate | Adopt upstream's version if merged |
| `src/stores/popup-queue-store.ts` | New: central popup arbitration — modal lane (one at a time, quiet period, defers while chat streams) + banner lane (priority preemption) | Not yet submitted upstream — good PR candidate | Adopt upstream's version if merged |
| `src/components/notification-bell.tsx` | New: floating bell + inbox panel, renders only while unread items exist | Not yet submitted upstream — good PR candidate | Adopt upstream's version if merged |
| `src/lib/z-layers.ts` | New: named z-index layer tokens (banner/bell/modal/toast/critical) | Not yet submitted upstream | Keep in sync with overlay components |
| `src/components/ui/toast.tsx` | Added optional `action` button to toasts (used by "Updates available → See what's new") | Not yet submitted upstream | Small additive change |
| `src/components/claude-reconnect-banner.tsx` | Visibility now goes through the banner slot lane (priority 100) + z-layer token | Not yet submitted upstream | Re-apply slot gating if upstream rewrites the banner |
| `src/components/openrouter-credits-banner.tsx` | Visibility now goes through the banner slot lane (priority 50, preempted by reconnect) + z-layer token | Personal (banner itself is ours) | Keep |
| `src/routes/settings/index.tsx` | Added "Popups & Inbox" section under Notifications (3 toggles backed by notification-center-store) | Not yet submitted upstream | Merge carefully if upstream restructures settings sections |
...
| `src/screens/settings/harp-config-screen.tsx` | Full HARP tiered routing config screen + `CapWidget` for day/week/month cap selector; **+ Route Combos editor (2026-09) — mirrors the tier-model editor: enabled/enforce toggles, per-combo task/risk match, reorderable `provider/model` steps** | Part of PR #626 | Compare with upstream's version if merged — adopt theirs if feature-complete; **the Route Combos section is greenfield (not in PR #626) — keep on conflict** |
| `src/server/harp-config-store.ts` | New server module: HARP config read/write/patch with multi-path auto-discovery; **+ `combos:` block support (2026-09) — `HarpCombosConfig`/`HarpComboEntry` types, 8 `HarpPatch` variants (`set-combos-global`, `add/remove/rename-combo`, `set-combo-match`, `add/remove/reorder-combo-step(s)`), `normalizeCombos`, `default:` guard on the patch switch** | Part of PR #626 | Adopt upstream's version if merged; **the combo types + patch cases are greenfield (not in PR #626) — keep on conflict** |
| `src/server/harp-config-store.test.ts` | **NEW (2026-09)** — vitest for the combo patch cases + `normalizeCombos` + unknown-action rejection | Not in PR #626 | Keep |
...
| `src/screens/command/command-center-screen.tsx` | NEW — Command Center: unified agent roster (AI sisters + delegation profiles), swarm personality roster, latest snapshot regression card, System Health card, Sister Readiness card, inline Run now/View jobs/Open logs actions, one-click Operations Snapshot JSON export, archive link, Agent Bus panel, recent activity; replaces separate Agents + Operations nav entries | Personal customization | Keep always |
| `src/lib/ops-snapshot-regression.ts` | NEW — shared TypeScript helper for Command Center and /snapshots regression detection, deltas, and evidence-log routing | Personal customization | Keep always |
| `src/routes/logs.tsx` | NEW — /logs Operational Logs screen: safe tail viewer for local Hermes/Workspace/cron/VM/HARP logs with URL-addressable `source` selector, URL-addressable severity/time filter (`all/errors/warnings/5m/1h`), line counts, copy/download actions, source-path copy, clear-filter action, and refresh action | Personal customization | Keep always |
| `src/routes/snapshots.tsx` | NEW — /snapshots Operations Snapshot Archive screen: latest snapshot, regression/attention banner with evidence-log link, 24h health/readiness trend totals, dependency-free SVG trend chart, retention visibility/prune action, snapshot comparison/diff export, recent snapshot list, and JSON downloads | Personal customization | Keep always |
| `src/routes/api/ops-logs.ts` | NEW — GET /api/ops-logs: authenticated allowlisted log tail API with bounded tail reads, secret redaction, and latest cron-output discovery | Personal customization | Keep always |
| `src/routes/api/ops-snapshots.ts` | NEW — GET/POST /api/ops-snapshots: authenticated allowlisted snapshot archive listing, retention metadata/prune action, and single-snapshot download API | Personal customization | Keep always |
| `src/routes/api/ops-snapshot.ts` | NEW — GET /api/ops-snapshot: authenticated reusable operations snapshot endpoint for Command Center, cron, terminal automation, and future Astra/Telegram commands | Personal customization | Keep always |
| `/srv/projects/_hermes-control/scripts/export-ops-snapshot.py` + `~/.hermes/profiles/novus/scripts/export-ops-snapshot.sh` + cron `ops-snapshot-archive` | NEW — hourly script-only operations snapshot archive under `/srv/projects/_hermes-control/snapshots/ops` with bounded log previews, retention pruning, and quiet regression-only stdout alerts | Local ops automation | Keep on this VM |
| `src/routes/api/system-health.ts` | NEW — GET /api/system-health: runs the local Hermes watchdog script and returns gateway/dashboard/workspace/Odysseus/disk/memory/cron health JSON for Command Center | Personal customization | Keep always |
| `src/routes/api/sister-readiness.ts` | NEW — GET /api/sister-readiness: runs the local sister readiness audit script and returns profile/growth/routing health JSON for Command Center | Personal customization | Keep always |
...
| `src/components/{dashboard-overflow-panel,mobile-hamburger-menu,mobile-tab-bar}.tsx` + `src/hooks/use-swipe-navigation.ts` + `src/routes/{index,$}.tsx` + `src/screens/chat/components/chat-sidebar.tsx` + `src/screens/tasks/tasks-screen.tsx` | AUDIT 2026-07 — TanStack Router typecheck cleanup: dynamic navigations now pass explicit empty `search`, chat links use literal routes, and task-dialog close preserves typed search params | Upstream PR candidate if upstream hits same route-gen strictness | Keep unless upstream makes equivalent typed-navigation fixes |
| `src/styles.css` | AUDIT 2026-07 — `@theme` now declares `--color-surface/-ink/-primary-50..950/-accent-50..950` so Tailwind v4 generates the utilities the whole app already uses (they compiled to nothing before); plus 3 light themes' `--theme-muted` darkened to WCAG AA 4.5:1 | **PR #673 submitted** (identical bug upstream) | If upstream merges an equivalent @theme fix, drop the token half of our commit; keep the AA muted tweaks (theme values differ per fork) |
| `src/styles.css` | 2026-09 loading-anim rebuild — removed a duplicate `@keyframes shimmer` + orphaned `.animate-shimmer` (theme-aware def at "Shimmer skeleton" is now the single source); added `--motion-fast/-base/-slow/-skeleton` to `:root` (deliberately NOT the `@theme` block owned by PR #673, to keep that PR's conflict surface narrow); new `.refresh-bar` affordance + its `prefers-reduced-motion` fallback | Personal customization | Keep; independent of PR #673 (different block) |
| `src/screens/dashboard/components/widget-shell.tsx` + `dashboard-screen.tsx` | 2026-09 loading-anim rebuild — `WidgetShell` gains `loading?` → renders `<WidgetSkeleton id={id}>` in place of the card body on first load only; dashboard-screen gates every `WidgetShell` on `overviewQuery.isPending` / `sessionsQuery.isPending` (never `isFetching`), adds `placeholderData: keepPreviousData` to the overview + sessions queries, and shows a `.refresh-bar` while a background refetch is in flight | Not yet submitted (dashboard diverged) | Keep; compare if upstream reworks dashboard loading |
| `src/components/workspace-shell.tsx` | AUDIT 2026-07 — persistent terminal container's `bottom` is `var(--tabbar-h)` on mobile instead of `inset:0`, so the mobile input bar sits above the fixed tab bar | **PR #674 submitted** | Drop ours if upstream merges the same fix |
| `src/screens/swarm2/operational-worker-card.tsx` | AUDIT 2026-07 EXTEND — header wraps to two rows below `md` (was absolute-positioned overlap); role badge gets `max-w-full`/`min-w-0` so truncate works (was overflowing leftward at all viewports) | **PR #675 submitted** | Drop ours if upstream merges the same fix; keep the PersonalityBadge parts regardless (personal) |
| `src/routes/files.tsx` | AUDIT 2026-07 — empty-state `<h1>Files</h1>` hidden below `md` (duplicated the shell's MobilePageHeader title) | **PR #676 submitted** | Drop ours if upstream merges the same fix |
