...
const baseUrl = (
  mergedEnv.BINANCE_BASE_URL || 'https://api.binance.com'
).replace(/\/$/, '')
...
if (!apiKey || !apiSecret) {
  console.error(
    'Missing production Binance credentials. Add BINANCE_API_KEY and BINANCE_API_SECRET to hermes-workspace/.env for a live read-only account check.',
  )
  process.exit(2)
}
...
if (!liveReadApproval) {
  console.error(
    'Refusing live Binance account access without BINANCE_ALLOW_LIVE_ACCOUNT_READ=I_APPROVE_LIVE_ACCOUNT_READ. This script is read-only but still touches the production account API.',
  )
  process.exit(2)
}
...
if (baseUrl.includes('testnet')) {
  console.error(
    'Refusing testnet URL in live read-only smoke script. Use scripts/binance-testnet-smoke.mjs for testnet/demo keys.',
  )
  process.exit(2)
}
...
async function main() {
  ...
  const apiRestrictions = await requestJson(
    `${baseUrl}/sapi/v1/account/apiRestrictions?${signedParams(apiSecret).toString()}`,
  ...
  if (unsafeFlags.length > 0) {
    throw new Error(
      `Unsafe Binance API key permissions: ${unsafeFlags.join(', ')}. For this phase keep only Reading enabled; spot trading can be added later after Hermes approval/risk controls are enabled.`,
    )
  }
  const account = await requestJson(
    `${baseUrl}/api/v3/account?${signedParams(apiSecret).toString()}`,
    { headers: { 'X-MBX-APIKEY': apiKey } },
  )
    ...
    JSON.stringify(
      {
        ...
        apiKeyPermissions: {
          ...
          universalTransferEnabled:
            apiRestrictions?.permitsUniversalTransfer ?? null,
        },
