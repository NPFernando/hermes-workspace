...
const apiKey = mergedEnv.BINANCE_TESTNET_API_KEY || mergedEnv.BINANCE_API_KEY
const apiSecret = mergedEnv.BINANCE_TESTNET_API_SECRET || mergedEnv.BINANCE_API_SECRET
const baseUrl = (mergedEnv.BINANCE_TESTNET_BASE_URL || 'https://testnet.binance.vision').replace(/\/$/, '')

if (!apiKey || !apiSecret) {
  console.error('Missing Binance testnet credentials. Add BINANCE_TESTNET_API_KEY and BINANCE_TESTNET_API_SECRET to hermes-workspace/.env, then restart hermes-workspace.service.')
  process.exit(2)
}
