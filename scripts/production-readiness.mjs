...
function artifactBuildId() {
  try {
    return createHash('sha256')
      .update(readFileSync(join(rootDir, 'dist/server/server.js')))
      .digest('hex')
      .slice(0, 16)
  } catch {
    return null
...
async function deploymentIdentity() {
  ...
  const served = await fetch(`${baseUrl}/`, { cache: 'no-store' })
...
async function serviceCheck() {
  const active = await command('systemctl', [
    'is-active',
    'hermes-workspace.service',
  ])
  ...
  return result(
    active.ok && active.stdout === 'active' ? 'pass' : 'fail',
    active.stdout || active.stderr || 'Service status unavailable',
    { service: 'hermes-workspace.service', pid: pid.stdout || null },
  )
}
...
async function testCheck(skipTests) {
  ...
  return result(
    ...
    test.ok
      ...
      : test.stderr || 'Repository test suite failed.',
    {
...
async function securityCheck() {
  ...
  if (!gh.ok)
    return result('unavailable', 'GitHub CLI is not installed or unavailable.')
  const remote = await command('git', ['remote', 'get-url', 'origin'])
  const slug = repositorySlug(remote.stdout)
  if (!slug)
    return result(
      'unavailable',
      'A GitHub repository could not be derived from origin.',
    )
  ...
  const dependabotDisabled = /Dependabot alerts are disabled/i.test(
    dependabot.stderr,
  )
  let codeqlRun = null
  try {
  ...
  const codeqlRunEvidence = codeqlRun
    ? {
        status: 'pass',
        databaseId: codeqlRun.databaseId ?? null,
        headSha: head.stdout || null,
      }
  ...
  if (dependabotOpen == null) {
    return result(
      'unavailable',
      'GitHub Dependabot alert API was not available.',
      {
        codeqlOpen,
        dependabotOpen,
...
async function migrationCheck() {
  ...
  const postgres = existsSync(
    join(rootDir, 'src/server/finance-postgres-store.ts'),
  )
  const migrationDocs = existsSync(
    join(rootDir, 'docs/finance-cutover-open-questions.md'),
  )
  const complete = schema && store && postgres
  return result(
    complete ? 'degraded' : 'fail',
    complete
      ? 'Migration artifacts exist; live migration state requires operator/database evidence.'
      : 'Required finance persistence artifacts are missing.',
    { schema, store, postgres, migrationDocs, evidence: 'filesystem-only' },
  )
}
...
async function backupCheck() {
  ...
  const unit = existsSync(
    join(rootDir, 'deploy/systemd/hermes-finance-offsite-backup.service'),
  )
  const timer = existsSync(
    join(rootDir, 'deploy/systemd/hermes-finance-offsite-backup.timer'),
  )
  ...
  const enabled = await command(
    'systemctl',
    ['is-enabled', 'hermes-finance-offsite-backup.timer'],
    { timeout: 5_000 },
  )
  const active = await command(
    'systemctl',
    ['is-active', 'hermes-finance-offsite-backup.timer'],
    { timeout: 5_000 },
  )
  const evidence = await command(
    ...
    [
      '-u',
      'hermes-finance-offsite-backup.service',
      '--since',
      '30 days ago',
      '--no-pager',
      '-g',
      'roundTripVerified|"ok": true',
    ],
    { timeout: 10_000 },
  )
  const complete =
    ...
    Boolean(evidence.stdout)
  const missing = [
    !script && 'backup script',
    !unit && 'backup service unit',
    !timer && 'backup timer unit',
    !passphrase && 'backup passphrase',
    !remote && 'rclone remote',
    !rclone.ok && 'rclone binary',
    enabled.stdout !== 'enabled' && 'enabled timer',
    active.stdout !== 'active' && 'active timer',
    !evidence.stdout && 'recent round-trip evidence',
  ].filter(Boolean)
  return result(
    complete ? 'pass' : 'fail',
    complete
      ? 'Encrypted off-site backup is configured, scheduled, and has recent round-trip evidence.'
      : `Encrypted off-site backup is not ready: missing ${missing.join(', ')}.`,
    {
      ...
      evidence:
        'configuration, systemd, binary, and journal checks; secret values excluded',
    },
  )
}
...
async function credentialRotationCheck() {
  ...
  if (!audit.ok)
    return result(
      'unavailable',
      'Credential rotation metadata could not be read.',
    )
  let statuses
  try {
    statuses = JSON.parse(audit.stdout).status
  } catch {
    return result(
      'unavailable',
      'Credential rotation metadata returned invalid JSON.',
    )
  }
  if (!Array.isArray(statuses))
    return result(
      'unavailable',
      'Credential rotation metadata has no status list.',
    )
  ...
  const untracked = statuses.filter((entry) => entry.state === 'untracked')
  const status =
    statuses.length === 0 ||
    expired.length > 0 ||
    expiring.length > 0 ||
  ...
  return result(
    status,
    status === 'pass'
      ? 'Tracked credentials have valid rotation metadata.'
      : statuses.length === 0
        ? 'No credential rotation metadata has been recorded.'
        : `Credential rotation needs attention: ${expired.length} expired, ${expiring.length} expiring, ${unconfigured.length} not configured, ${untracked.length} configured without rotation metadata.`,
    {
      tracked: statuses.length,
      expired: expired.map((entry) => entry.key),
      expiring: expiring.map((entry) => ({
        key: entry.key,
        daysRemaining: entry.daysRemaining,
      })),
      unconfigured: unconfigured.map((entry) => entry.key),
      untracked: untracked.map((entry) => entry.key),
      statuses,
...
async function forkSyncCheck() {
  ...
  if (!slug)
    return result(
      'unavailable',
      'A GitHub repository could not be derived for fork-sync readiness.',
    )
  const metadata = await command(
    'gh',
    ['repo', 'view', slug, '--json', 'isFork,parent'],
    { timeout: 10_000 },
  )
  if (!metadata.ok)
    return result('unavailable', 'GitHub fork metadata was not available.')
  let parsed
  try {
    parsed = JSON.parse(metadata.stdout)
  } catch {
    return result('unavailable', 'GitHub returned invalid fork metadata.')
  }
  if (!parsed.isFork)
    return result(
      'pass',
      'Fork synchronization is not applicable because this repository is not a fork.',
      { isFork: false, applicable: false },
    )

  const configPath =
    process.env.HERMES_FORK_SYNC_CONFIG ||
    '/home/ubuntu/.hermes/fork-sync-preview.env'
  const config = existsSync(configPath)
  const timerEnabled = await command(
    'systemctl',
    ['is-enabled', 'hermes-fork-sync-preview.timer'],
    { timeout: 5_000 },
  )
  const timerActive = await command(
    'systemctl',
    ['is-active', 'hermes-fork-sync-preview.timer'],
    { timeout: 5_000 },
  )
  const applicable =
    config &&
  ...
  return result(
      ...
      : 'Fork synchronization is applicable but its preview config/timer is not ready.',
    {
      ...
      applicable: true,
      config,
      timerEnabled: timerEnabled.stdout === 'enabled',
      timerActive: timerActive.stdout === 'active',
      parent: parsed.parent?.fullName ?? null,
    },
  )
}
...
export async function buildReadinessReport({
  ...
  try {
    ...
    try {
      ...
      assets = result(
        'pass',
        'All local HTML asset references are available.',
        assetReport,
      )
    } catch (error) {
      assets = result(
    ...
    const warnings = Object.entries(checks)
      .filter(([, check]) =>
        ['degraded', 'unavailable', 'not-run'].includes(check.status),
      )
      .map(([name, check]) => `${name}: ${check.detail}`)
    return {
...
if (invokedScript === fileURLToPath(import.meta.url)) {
  ...
  if (process.argv.includes('--json'))
    ...
    for (const [name, check] of Object.entries(report.checks))
      console.log(
        `${check.status === 'pass' ? '✓' : check.status === 'fail' ? '✗' : '⚠'} ${name}: ${check.detail}`,
      )
