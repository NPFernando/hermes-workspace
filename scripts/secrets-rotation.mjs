...
export const DEFAULT_KEYS = [
  ...
  'AUTH_E2E_PASSWORD',
  'HERMES_PG_PASSWORD',
  'HERMES_FINANCE_BACKUP_PASSPHRASE',
  'HERMES_FINANCE_BACKUP_RCLONE_REMOTE',
  'OPENAI_API_KEY',
  'ANTHROPIC_API_KEY',
  'OPENROUTER_API_KEY',
  'CLAUDE_API_TOKEN',
...
const defaultMetadataPath = resolve(
  process.env.HERMES_SECRET_ROTATION_FILE || '.runtime/secret-rotation.json',
)
const defaultAuditPath = resolve(
  process.env.HERMES_SECRET_ROTATION_AUDIT_FILE ||
    '.runtime/secret-rotation-audit.jsonl',
)
...
export function recordRotation(
  metadata,
  { key, owner, source, expiresAt, rotatedAt = new Date().toISOString() },
) {
  if (!/^[A-Z][A-Z0-9_]{2,127}$/.test(key))
    throw new Error('key must be an uppercase environment variable name')
  if (!owner?.trim() || !source?.trim())
    throw new Error('owner and source are required')
...
export function rotationStatus(
  metadata,
  { configuredKeys = [], now = Date.now(), warningDays = 14 } = {},
) {
  const warningMs = warningDays * 24 * 60 * 60 * 1000
  const tracked = Object.entries(metadata.secrets).map(([key, entry]) => {
  ...
  const trackedKeys = new Set(Object.keys(metadata.secrets))
  const untracked = configuredKeys
...
export function writeRotation(
  ...
  appendFileSync(
    auditPath,
    `${JSON.stringify({ at: new Date().toISOString(), action: 'record_rotation', ...event })}\n`,
    { mode: 0o600 },
  )
...
export function configuredSecretKeys() {
  ...
  const configured = new Set(
    DEFAULT_KEYS.filter((key) => Boolean(process.env[key])),
  )
...
if (import.meta.url === `file://${process.argv[1]}`) {
  ...
  if (!command || command === 'status') {
    ...
    const next = recordRotation(metadata, {
      key,
      owner: value('--owner'),
      source: value('--source'),
      expiresAt: value('--expires-at'),
      rotatedAt: value('--rotated-at') || undefined,
    })
    writeRotation(next, {
      event: {
        key,
        owner: next.secrets[key].owner,
        source: next.secrets[key].source,
        expiresAt: next.secrets[key].expiresAt,
      },
    })
    console.log(
      JSON.stringify({
    ...
    console.error(
      'Usage: secrets-rotation.mjs [status|record KEY --owner=... --source=... --expires-at=ISO]',
    )
    process.exitCode = 2
  }
}
