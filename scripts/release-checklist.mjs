...
export async function runReleaseChecklist({ baseUrl = 'http://127.0.0.1:3000', service = process.env.HERMES_SERVICE_NAME || 'hermes-workspace', expectedBuild = process.env.RELEASE_SMOKE_EXPECTED_BUILD } = {}) {
  const checks = []
  const pwa = checkPwa(process.cwd())
  if (!pwa.ok) throw new Error(`PWA integrity failed: ${pwa.failures.join('; ')}`)
...
if (import.meta.url === `file://${process.argv[1]}`) {
  try {
    console.log(JSON.stringify(await runReleaseChecklist({ baseUrl: process.argv[2] || 'http://127.0.0.1:3000' }), null, 2))
  } catch (error) {
    console.error(`release checklist failed: ${error instanceof Error ? error.message : String(error)}`)
    process.exitCode = 1
  }
}
