...
The next candidate is **PF-208** (FX Source/Freshness Metadata — add `currency`/`exchangeRateSource` to the Tax records `DataTable` columns, same shape as TAX-103/104 but lower value). Beyond that, this "surface an already-computed value" pattern looks close to fully mined across every phase audited this session (Phase 0/1/4/8/10/36/37/41/42/43/44 plus this pass's Phase 2/5/33/34) — a future scoping pass may need to accept real design/build work rather than another pure-surfacing pass.
