...
| C | planned | The actual read-cutover — `readFinanceStore()` assembles personal-finance-domain fields from Postgres instead of JSON; `writeFinanceStore()` persists them directly. Heavy live verification across every panel. |
| D | planned | Cleanup — stop writing personal-finance fields to JSON at all; freeze the last JSON snapshot as a rollback-only artifact (HARP's own proven pattern). |
