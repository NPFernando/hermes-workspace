...
- **What was built**: `buildFinanceAnswerMarkdown(question, answer, chart)` (new, unit-tested, `src/screens/personal-finance/utils.ts`) turns the live Finance Analyst answer into a shareable markdown report (question, answer, an optional chart-data table with `formatLkr`-formatted values, and an export-timestamp footer). `FinanceAnalystCard` gets "Copy" and "Download .md" buttons reusing the exact Blob-download (`new Blob → URL.createObjectURL → temp <a download> → click → revokeObjectURL`) and clipboard-copy (`navigator.clipboard.writeText`, with a hidden-textarea fallback) shape already proven by `ExportMissionButton` (`src/screens/gateway/components/export-mission.tsx`) — the only existing export convention anywhere in this repo, confirmed via research before building (no server-side "generate report" action exists anywhere).
- **Key design decision (100% client-side, nothing persisted)**: this exports only what's already visible in the component's live state — no new `FinanceSettings` field, no new route action, no Postgres/mirror changes. Matches AI-203's "chart is ephemeral" treatment.
- **Known limitation**: the "✓ Copied" button-label flip couldn't be reliably asserted in headless Playwright — isolated testing traced it to a CDP/clipboard-permission-override quirk specific to automated browsers (a plain synchronous or generic-async state update flips the label fine; only the update immediately after `navigator.clipboard.writeText()`'s promise doesn't visibly commit in that environment). The clipboard content itself is correct every time, and this exact async-then-setState pattern is already shipped and in use via `ExportMissionButton` — treated as a testing-environment artifact, not a product bug.
- **Verified live**: asked a question, clicked "Download .md" — file downloaded with the expected `finance-answer-YYYY-MM-DD.md` filename and correct question/answer/footer content; clicked "Copy" — clipboard correctly received the full markdown export (verified via a sentinel-value-then-overwrite check). `financeQaHistory` reset afterward.
...
### Shipped: AI-600 — Account Balance Reconciliation (Phase 28, first slice)

- **What was built**: `computeAccountLedgerBalance()` (new, unit-tested, `src/screens/personal-finance/utils.ts`) compares an account's manually-maintained `balance` against `openingBalance` plus the sum of its own same-currency tagged income/expense transactions. Returns `null` when there's no `openingBalance` set — shown as an informational "Set an opening balance to enable reconciliation" hint rather than a misleading computed number. `AccountsPanel` shows a per-account badge: "✓ Reconciled", "⚠ Off by X", or the hint — informational only, no auto-correction of either number.
- **Key correction (Phase 28 was over-flagged as blocked)**: the phase-level "needs a real ledger" blocker is real for AI-601-607 (which would need a genuine external bank-feed/statement source of truth), but AI-600 needed none of that — `FinanceAccount.balance`/`openingBalance` and income/expense `accountId` already existed in the data model and had simply never been compared against each other. `accounts-panel.tsx` itself had a stale comment anticipating this exact feature under an orphaned ID ("PF-104") that was never actually built under that name — that ID got reused elsewhere (`getUnifiedTransactions()`) with no cross-reference update, a case worth remembering when trusting old in-code "future work" comments at face value.
- **Real bug found during test-data construction**: `IncomeRecord` uses `originalCurrency`/`originalAmount` fields, not `currency`/`amount` like `ExpenseRecord` — the first draft of the income-to-ledger-transaction mapping in `accounts-panel.tsx` used the wrong field names (silently reading `undefined`/`0` for every income record) and was caught only because constructing realistic injected test income data required checking `IncomeRecord`'s real shape first.
- **Key design decision**: only same-currency transactions are summed (no FX conversion attempted) — a documented known limitation, not a silent inaccuracy, since cross-currency-tagged transactions are rare and conversion-rate handling would add real complexity for a first slice.
- **Verified live** (3 injected test accounts — reconciled, deliberately mismatched, and no-opening-balance — plus tagged income/expense records, since this environment normally has none): all three states rendered correctly, including the exact "Off by LKR 89,999" diff amount for the mismatched case. Test accounts/records removed afterward.

### Shipped: WEALTH-108 — Estate/Beneficiary Notes

- **What was built**: a new `Beneficiary` entity (`id`, `name`, `relationship`, optional `note`) — the third and simplest instance of this session's "six-file new top-level entity" pattern (after `Loan`/`Property`): CRUD branches in `addFinanceRecord`/`updateFinanceRecord`/`deleteFinanceRecord`, `FinanceDatabase.beneficiaries` + default init, `mirrorIntoSplitStores()`/`overlaySplitStores()` wiring, a `beneficiaries` table in `personal-finance-postgres-store.ts`, and a new `BeneficiariesPanel` mounted alongside `LoansPanel`/`PropertiesPanel` in the Investments tab.
- **Key design decision (zero calculation involvement)**: unlike `Loan`/`Property` (which feed `debtLkr`/`propertyValueLkr`/`netWorthLkr`), `Beneficiary` touches no summary math at all — no status enum, no numeric fields, nothing added to `financeSummary()`. Matches Naveen's explicit "purely informational, not legally binding" scoping.
- **Correctness fix along the way (not a scope change)**: `PERSONAL_FINANCE_RECORD_KINDS` (`routes/api/finance.ts`) was never updated when `loan`/`property` were added, so their mutations return the heavier legacy `financePayload()` shape instead of the lean `personalFinancePayload()` — harmless (both include `.data.loans`/`.data.properties`) but inconsistent. `'beneficiary'` was added to that set correctly for this new code; the pre-existing `loan`/`property` gap was left alone (out of scope for this task).
- **Verified live**: added a beneficiary (name/relationship/note), confirmed it appeared immediately; edited it, confirmed the change persisted after a page reload; confirmed `netWorthLkr` (376,437 LKR) was exactly unchanged before and after adding/editing/deleting it; deleted it and confirmed removal.

### Shipped: Postgres Migration Phase A — schema completion

- **What was built**: real Postgres tables for the 4 personal-finance collections that had zero Postgres backing before this — `pending_ingestions` normalized into a parent table plus `pending_ingestion_extracted_transactions`/`pending_ingestion_extracted_contracts`/`pending_ingestion_contract_risks` child tables with real foreign keys (instead of embedding the AI extraction blobs as loose JSON), `exchange_rates` (structured `base_currency/target_currency/date/rate`), `investment_accounts` (kept generic — a JSONB `data` column, since no CRUD path or defined shape exists anywhere in the app for this collection today), and a new settings-tables group (`personal_finance_settings`, `finance_qa_history`, `gmail_ingest_state`, `gmail_sync_history`, `category_correction_hints`) for the personal-finance-owned subset of the shared `FinanceSettings` bag.
- **Key context**: this is Phase A of a multi-phase project (see the new "Postgres Migration Project" section near the top of this doc) to make personal finance fully Postgres-primary. Today's change is dual-write only — every read still comes from JSON, zero behavior change to the running app.
- **Key design decision (settings split)**: `FinanceSettings` is genuinely shared between personal-finance and trading-engine concerns. Only the personal-finance-owned fields (`emergencyFundTargetMonths`, `savingsRateTargetPct`, `wealthGoalTargetLkr`/`wealthGoalTargetDate`, `financeQaHistory`, `gmailIngest`, `categoryCorrections`) got new tables. The trading-shared remainder stays exactly where it is today, mirrored through `finance-postgres-store.ts` — untouched, per the standing constraint.
- **Verified live**: triggered a real mirror write and confirmed via `psql` against the `personal_finance` database that all 27 tables (16 existing + 11 new) receive correct data with correct relational integrity — the 7 real historical contract extractions' 45 risk rows all correctly join back to their parent contract with zero orphans; setting/clearing savings/wealth targets, Finance Analyst history, category corrections, and Gmail sync state round-tripped correctly in both directions.

### Shipped: Postgres Migration Phase B — read reconstruction

- **What was built**: `readPersonalFinancePostgresStore()` (`personal-finance-postgres-store.ts`) reconstructs the full `PersonalFinanceSlice` shape from all 27 tables. `selectJson()` has Postgres itself serialize each query's result set to JSON (`json_agg`) rather than hand-parsing `psql`'s tabular text output — avoids fragile delimiter/quote/null parsing entirely, the same trick `finance-postgres-store.ts` already uses for its own JSONB snapshot. `snakeRowToCamel()` converts column names back to the JS field names, without recursing into opaque nested payloads (`investment_accounts.data`).
- **Key design decision (not wired into any live read path)**: `overlaySplitStores()` (`finance-store.ts`) still reads exclusively from JSON — zero behavior change to the running app. This function exists purely to be built and proven correct in isolation before Phase C ever calls it for a real read; keeping it disconnected means Phase B carries zero risk to the running app regardless of any bug in the new code.
- **Known limitation**: this file's existing test guard (`VITEST`/`NODE_ENV=test` disables real Postgres calls) means genuine round-trip verification is a live, manual pass rather than an automated CI test — consistent with this file having zero automated tests before this phase. Only the pure `snakeRowToCamel()` key-converter got a real unit test.
- **Verified live**: wrote real + injected test data across every table category Phase A introduced (a full extracted contract + its risks, exchange rates, an investment account, settings targets, Finance Analyst history, a category correction, Gmail sync state) via the existing mirror, read it all back through the new function, and confirmed every field matched exactly — including the previously-untested `exchange_rates`/`investment_accounts`/settings-subfield paths, since the pre-existing real data (accounts, pending_ingestions) didn't exercise them. Test data removed by exact id afterward.

### Shipped: Postgres Migration Phase C — read cutover

- **What was built**: `overlaySplitStores()` (`finance-store.ts`) now tries `readPersonalFinancePostgresStore()` first for personal-finance collections and settings — the actual moment personal finance became Postgres-primary in the live app. Three-tier fallback: Postgres → JSON split store (existing freshness check, unchanged) → base file. A new `HERMES_PERSONAL_FINANCE_READ_SOURCE=json` env var disables the Postgres read path instantly without a redeploy.
- **Key design decision (writes unchanged)**: this phase touches reads only — `writeFinanceStore()`/`mirrorIntoSplitStores()` are untouched, still writing to the base JSON file, the JSON split store, and Postgres every time. This keeps the JSON split store a fully valid, currently-accurate fallback throughout Phase C, not a decaying snapshot — Phase D (stopping JSON writes) deliberately waits until this phase has run stable for a while.
- **Verification approach**: given the blast radius (100+ call sites across 25 files depend on `readFinanceStore()`), verification leaned on an automated equivalence script rather than manually clicking through every panel — it confirmed `readFinanceStore()` produces byte-identical results for every personal-finance field and settings sub-field whether read via Postgres or via the kill switch's JSON-only path, against real production data. New unit tests (mocked Postgres read) cover the Postgres-priority/JSON-fallback/kill-switch behavior directly.
- **Verified live**: the equivalence script passed against real data (8 real historical `pending_ingestions`, live settings, real stock holdings/fixed deposits); a Playwright pass confirmed every Investments-tab panel and net worth (376,437 LKR, matching the known baseline) render correctly through the new read path; confirmed the pending-ingestions "Nothing pending" empty state on the Ingestion tab was correct existing UI filtering (only `awaiting_review`/`awaiting_password` items show there), not a data gap — the 8 real `confirmed`/`rejected` items were confirmed present and correct via a direct `readFinanceStore()` check.

### Shipped: Postgres Migration Phase D — cleanup (project complete)

- **What was built**: retired the JSON split-store mirror for personal finance — `~/.hermes/finance/personal-finance.json` was frozen read-only as a rollback snapshot (`personal-finance.json.frozen-phaseD-20260902`, `chmod 400`) and nothing writes to it anymore. `mirrorIntoSplitStores()` now calls `writePersonalFinancePostgresStore()` directly instead of through the retired `writePersonalFinanceStore()`. `overlaySplitStores()` collapsed from a three-tier fallback (Postgres → JSON split store → base file) to a clean two-tier one (Postgres → base file) — the middle tier was removed because it would otherwise silently serve increasingly stale data forever once nothing refreshes it. Deleted the now-dead `writePersonalFinanceStore()`/`readPersonalFinanceStore()` functions (confirmed zero other callers); `personal-finance-store.ts` now holds only the shared `PersonalFinanceSlice` type.
- **Key insight (the base file doesn't need a dedicated mirror-write step)**: the base `finance.json` file never goes stale for personal-finance fields despite no code specifically maintaining it for that purpose — `overlaySplitStores()` already populates the in-memory `db` object with fresh Postgres data on every read, and the app's ordinary read→mutate→write cycle then serializes that already-current object straight back to disk. This is why removing the middle fallback tier was safe rather than a coverage gap.
- **Timing**: Phase C's own plan explicitly scoped Phase D to wait for a real observation period first (matching HARP's own cutover cadence). Naveen asked to proceed immediately instead — done, with the same live-equivalence-check rigor as Phase C rather than skipping verification to move faster.
- **Verified live**: re-ran the Phase C equivalence check (Postgres vs. kill-switch reads byte-identical for every personal-finance field) **after** the split-store file was frozen and removed from its original path entirely; confirmed a real mirror write still correctly reaches Postgres (8 `pending_ingestions`, 7 extracted contracts) via the new direct call path; confirmed the frozen file's mtime never changes on subsequent writes.
- **Project status: complete.** All four phases (A: schema, B: read reconstruction, C: read cutover, D: cleanup) are shipped. Personal finance reads and writes go through Postgres as the real source of truth, with the base JSON file as an always-fresh safety net and one frozen point-in-time snapshot as a manual rollback reference — no active JSON mirror left to maintain.

### Shipped/Fixed: PF-416 — Responsive Dashboard Layout (corrected + fixed, step 1 of 3)

- **Context**: Naveen asked for the app to genuinely work well across web/Android aspect ratios, plus wants the existing Windows Electron build and Android PWA/TWA wrapper verified — agreed to do all three, in order, responsive design first. Research found the packaging infrastructure for both Windows (`electron/main.cjs`, `electron-builder.config.cjs`, `electron:build:win`) and Android (`public/manifest.json`, `public/sw.js`, a TWA `assetlinks.json` tied to package `com.fernandofamily.hermes`) already exists — neither needs to be built from scratch, only verified (steps 2 and 3, not yet done).
- **What was found (correcting a stale roadmap claim)**: PF-416 was previously marked `existing` with "Tailwind responsive grid throughout" — that doesn't match the code. Only 5 of 25 Personal Finance component files use any explicit Tailwind breakpoint (`sm:`/`md:`/`lg:`/`xl:`); the other 20 rely entirely on `flex-wrap` as a fallback.
- **What was actually verified (measure before fixing)**: rather than guessing which files were broken, a real Playwright multi-viewport audit (phone 360×800, foldable 673×841, tablet 768×1024, desktop 1280×800, all 5 tabs — 20 combinations) checked for actual horizontal page overflow. Result: **zero overflow cases** — the `flex-wrap` fallback genuinely works for page-level layout. The audit did find a real, different issue: at in-between widths like 673px, form-input flex items with no `min-width` shrink far enough that placeholder text gets clipped and fields cram onto fewer rows than they should.
- **What was fixed**: added `min-w-[140px]` to the exact shared `inputClass` string used identically across 13 panel components (accounts, loans, properties, beneficiaries, categories, subcategories/merchants, tags, transactions, budget, stock-holdings, fixed-deposits, income-sources, pending-ingestion), plus `min-w-[200px]` on `finance-analyst-card.tsx`'s single wide `flex-1` input. Fields now wrap onto new rows at narrow/in-between widths instead of compressing.
- **Known limitation / deliberate scope**: some placeholder text (e.g. "Opening balance (optional)") still visually truncates within a 140px-wide field even after wrapping cleanly — this is a content/copy question (shorten placeholder text), not a responsive-layout defect, and was left alone as out of scope for this pass per "fix only confirmed issues."
- **Verified live**: re-ran the same 20-combination audit after the fix — still zero overflow; visually confirmed via screenshot at 673px that the previous 6-fields-crammed-onto-2-rows layout now wraps into clean 2-3 field rows.
- **Next**: step 2 (verify the Windows Electron build works end-to-end) and step 3 (verify the Android PWA/TWA wrapper) remain, each as its own future plan-approval cycle.

### Verified (blocked): Windows Electron build — step 2 of 3
...
### Verified (passed): Android PWA/TWA wrapper — step 3 of 3
...
### Shipped: Theme-aware status colors (follow-up to the UI/UX cleanup pass)

- **Context**: the prior UI/UX cleanup pass (dead-code removal + duplication consolidation) deliberately left one finding out of scope: ~100+ hardcoded Tailwind color literals (`emerald-*`, `red-*`, `amber-*`, `sky-*`, `indigo-*`, plus a few raw hex codes in chart props) used for status pills, badges, and chart lines throughout the module. This picks that up as its own slice, per the plan noted at the time.
- **What was found on closer inspection**: the app ships 13 selectable themes (Matrix, Hermes/`claude-official`, Nous, Slate, Odysseus, two SciFi variants, etc). Only the two SciFi themes remap Tailwind's raw color tokens at the CSS-variable level (`scifi-theme.css` overrides `--color-emerald-500` and friends) — so the hardcoded literals happened to look theme-correct there, but rendered as disconnected default colors in every other theme. This was worse than "looks slightly off" in themes like Matrix (phosphor green) or Odysseus, where a hardcoded emerald/red/amber pill visually clashes with the rest of the UI.
- **What was built**: the four shared tone constants introduced in the prior pass (`shared-styles.ts`'s `positiveTone`/`warningTone`/`dangerTone`, plus a new `infoTone`) are now built from CSS `color-mix()` against `--theme-success`/`--theme-warning`/`--theme-danger`/`--theme-accent-secondary` — tokens every theme defines — instead of literal palette classes. A handful of one-off inline pill badges in `transactions-panel.tsx` (income/expense, pending/reconciled, sync-source) and the chart stroke/fill colors in `finance-trends-card.tsx`/`finance-analyst-card.tsx`/`personal-finance-screen.tsx` got the same treatment. `stat-card.tsx` (shared with Trading and Echo Studio, same hardcoded-color pattern) was fixed too, since the fix was identical and low-risk.
- **Real bug found along the way (not theme-related)**: the Overview tab's net-worth breakdown chart assigned the exact same color to the Cash, Stocks, and Fixed Deposits bars — genuinely indistinguishable regardless of theme. Each now gets its own theme-driven color (accent / accent-secondary / success / danger for Cash / Stocks / Fixed deposits / Debt).
- **Deliberately left alone**: the generic `bg-black/10`/`/20`/`/30` translucent-overlay classes used as a neutral surface tint across every input/pill/card (not a semantic status color) were not touched — they're a deliberate flat-black-overlay effect that reads consistently regardless of theme, and touching them would require visually verifying all 13 themes to be confident it's actually an improvement rather than a regression, which wasn't attempted this pass.
- **Verified**: `npx tsc --noEmit` and `npx eslint` clean; all 118 tests pass; production build succeeded in an isolated worktree and was deployed, verified 200 on both `localhost:3000` and `agent.fernandofamily.com`. A live cross-theme visual check (Playwright, switching themes and screenshotting) was attempted but blocked by this environment's auto-mode command classifier; not forced. Confidence instead rests on `color-mix(in srgb, ...)` being well-supported baseline CSS since 2023 in every evergreen browser, plus the mechanical nature of the change (every replacement is a literal-for-CSS-variable swap, same visual shape/opacity, verified by reading each diff).

### Resolved: Windows Electron build path (follow-up to the earlier blocked verification)

- **Context**: the earlier verification step found this dev VM (ARM64 Oracle Cloud) cannot build the Windows Electron app — electron-builder's rcedit tool needs a native x86 Windows host, which Wine-on-ARM64 can't provide without a full x86 emulation stack. Three options were left open for Naveen to decide: build on real x86_64 hardware/CI, set up x86 emulation on this VM, or accept win-arm64-only. He picked the CI-runner option.
- **What was built**: `.github/workflows/electron-build-windows.yml` — a `workflow_dispatch`-only GitHub Actions workflow that runs `electron:build:win` on a real `windows-latest` runner. Native x64 Windows means no Wine, no rcedit-ia32 problem, and `electron-builder.config.cjs`'s `win.target` (no arch override) now naturally produces a standard win-x64 build instead of the win-arm64 this dev VM was silently building. Explicitly passes `--publish never` since the existing `publish` config block points at the upstream `outsourc-e/hermes-workspace` repo, not this fork.
- **Verified live, not just "should work"**: triggered the workflow for real after merging (`gh workflow run`) rather than declaring it done on YAML inspection alone. All steps completed successfully in 5m37s, producing a genuine ~200MB build artifact (`hermes-workspace-windows`, containing the portable `.exe`/NSIS installer + `latest.yml`). One benign annotation (`git.exe` exit 128 during `actions/checkout`'s submodule-probing, which finds none and is standard noise for any repo without submodules) did not affect any step's success.
- **Project status: resolved.** The Windows build question raised in the earlier verification step is closed — a working, repeatable Windows build path now exists via manual GitHub Actions dispatch, with no changes needed to the app or its packaging config.

### Shipped: Theme-aware surface tint (follow-up to the theme-color pass)

- **Context**: the theme-color pass (#177) deliberately left the generic `bg-black/10`/`/20`/`/30` surface tint (used for inputs, buttons, and neutral status pills) untouched, pending a live visual check across themes — reasoning it might be a deliberate flat effect rather than a real inconsistency.
- **What the check found**: real Playwright screenshots against the live app, comparing a dark theme (Hermes/claude-official) against light themes (`claude-classic-light`, `claude-official-light`), confirmed the concern. Flat black-on-white reads as a plain muddy gray box in light/warm-paper themes — legible, not broken, but visually disconnected from the theme's own palette. The earlier caution to check before touching it was justified.
- **What was built**: `bg-black/10`/`/20`/`/30` (50 occurrences across 22 files) replaced with `bg-[color-mix(in_srgb,var(--theme-text)_N%,transparent)]` (8%/16%/24% respectively) — mixes in each theme's own high-contrast ink color instead of literal black, so the tint direction and hue both follow the active theme (lightens on dark themes, darkens with the theme's actual ink tone on light ones).
- **Verified live, before AND after merging**: screenshotted the same `claude-classic-light` Accounts panel before the fix (muddy gray) and after deploying the fix (subtle warm-charcoal tint matching the theme's own text color) — a direct visual before/after, not just a code-level assertion. Also re-confirmed `npx tsc --noEmit`/`npx eslint` clean and all 118 tests passing, production build succeeded in an isolated worktree, and `git diff --stat src/server/finance-postgres-store.ts` empty throughout.
- **Project status: the full UI/UX refinement + theme-fix arc from this session is now complete** — dead code removal, duplication consolidation, theme-aware status colors, and theme-aware surface tint are all shipped and live-verified.

### Shipped: Finished the theme-color conversion (fresh roadmap scoping pass finding)

- **Context**: a fresh scoping pass across all 45 phases, looking for a new feature candidate, instead found leftover work from the earlier theme-color pass (#177, #181) — those PRs converted the shared tone constants in `shared-styles.ts` but missed ~155 remaining hardcoded `emerald-*`/`red-*`/`amber-*` literals across 20 files, almost entirely Save/Delete/Cancel button styling and inline error/warning/amount text colors using the exact same pattern the shared constants already existed for.
- **What was built**: converted all ~155 occurrences to `color-mix()` against the theme tokens (same mechanical approach as the prior passes). Extracted 4 newly-identical Save/Delete button class strings (two size variants each) into `confirmButtonClass`/`confirmButtonClassLarge`/`dangerButtonClass`/`dangerButtonClassLarge` in `shared-styles.ts`, now used across 12 files instead of each hand-rolling the literal string.
