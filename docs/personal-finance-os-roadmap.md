...
**PF-113 — Pending/Cleared/Reconciled Status.** With Category/Subcategory/Merchant/Tag all shipped, this is the other item "Deferred by the PF-104 slice" — a `status: 'pending' | 'cleared' | 'reconciled'` field on `ExpenseRecord`/`IncomeRecord` (default `'cleared'` to match today's implicit behavior) plus a small status toggle/filter in `TransactionsPanel`. Unlike the four catalogue features, this is a plain enum field on the existing records, not a new entity — a smaller, more self-contained slice.
