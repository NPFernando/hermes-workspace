...
The last candidate from this Phase 4/8 pass is **PF-411 (Upcoming Money)**: no new data needed, but consolidating three already-working badge computations scattered across `payday-status.ts`/`income-sources-panel.tsx`/`fixed-deposits-panel.tsx` into one widget is more refactor surface than a single-file change — worth a bit more care in scoping than the last four slices. Once it ships, this phase's `partial` items are exhausted and a fresh registry pass (e.g. Phase 3 Personal Financial Rules, Phase 10 Goals/Emergency Funds, or Phase 42 Backup/Import) would be the next scoping exercise.
