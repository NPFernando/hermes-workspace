...
| D | not started | Cleanup — stop writing personal-finance fields to JSON at all; freeze the last JSON snapshot as a rollback-only artifact (HARP's own proven pattern). Deliberately waiting for Phase C to run stable for a real observation period first, matching HARP's own cutover cadence — not a "next slice," a "when it's earned it" step. |
