...
| 1 · window payload | ✅ done — `data.{income_records,expense_records}` capped to the trailing **36 months** (`TRANSACTIONS_WINDOW_MONTHS`); `financeSummary`'s all-time figures untouched (reads `db`); `TransactionsPanel` shows a "last 36 months" note with a **"Load full history"** button (pages the rest in via `list_transactions`, 500/page, and switches the panel to that snapshot until the next mutation) — PR-followups-paging; full history also in the JSON/CSV export | PR #46 |
...
| 12b · ledger-derived account balances (ADR-001) | ✅ done — `accounts-panel` already showed an AI-600 reconcile delta (`computeAccountLedgerBalance` = openingBalance + tagged income − expenses); it now also feeds each **transfer as two legs** (out of `fromAccountId`, into `toAccountId`) so transfer-touched accounts reconcile, and the "off by" line names the implied balance. `financeSummary` net-worth/cash **still reads the manual `balance`** — switching the source of truth is a separate sign-off call. | PR-followups-ledger-transfers |
| 12d · cross-currency reconcile legs (item 4) | ✅ done — `computeAccountLedgerBalance` (moved to `finance-store.ts`, `db`-first) now converts a leg in a different currency into the account's currency via `exchange_rates` (`convertBetweenCurrencies`); still excluded only when no rate is on file. The panel reads a server-computed `ledgerBalance` off each payload account row instead of re-deriving it client-side. | PR-ledger-networth |
| 12e · opt-in ledger net worth (item 3) | ✅ done (sign-off given via /goal) — `FinanceAccount.deriveBalanceFromLedger?: boolean` + a "Use ledger balance" checkbox on the account edit form. `financeSummary`'s `cashBalanceBase` and card `debtBase` use `effectiveAccountBalance(db, account)` = the ledger figure for opted-in accounts, **falling closed to the manual `balance`** when there's no opening balance (never 0, never dropped — asserted). Default (absent/false) = manual, unchanged. | PR-ledger-networth |
| 12f · transfer-aware list filter (item 5) | ✅ done — a **"Hide transfers"** checkbox in the `TransactionsPanel` filter row removes transfer rows from the rendered list independently of the single-select kind dropdown. Kept panel-local: `getBudgetVsActual` / `getFinanceTrends` are **not** threaded with an include-transfers flag (transfers would double-count a category budget); the trends card now says "Account transfers aren't counted". | PR-hide-transfers |
| U6 · currency-exposure block cryptic | ✅ done — "Money held in other currencies" with a plain sentence ("you hold value across N currencies outside {base}…") + a card per currency showing its total and the **breakdown by source** (`N active jobs · X`, `N holdings · X`, `N fixed deposits · X`). `getCurrencyExposure` now returns a `breakdown[]` per currency. | PR-dash-polish |
| U8 · no global freshness / manual refresh | ✅ done — `payload.checkedAt` (server build time, already computed) drives an **"Updated HH:MM · Refresh"** line under the hero; Refresh calls React Query's `refetch()`. | PR-dash-polish |
| NEW · net-worth history chart | ✅ done — `net_worth_snapshots` collection (in `FINANCE_COLLECTIONS`, guard-tested) + `recordNetWorthSnapshot(db)` (LKR, upsert-by-date, idempotent) + `snapshot_net_worth` action + nightly cron `9e4af7d8382b` (`50 23 * * *`, `personal-finance-snapshot.sh`, pre-deploy-safe). `payload.netWorthHistory` (last 180 pts, base currency) → `NetWorthHistoryCard` recharts line chart on Overview, hidden until ≥2 points. | PR-networth-history |
| NEW · CI typecheck gap | ✅ fixed — `.github/workflows/ci.yml` ran `pnpm run typecheck \|\| pnpm exec tsc --noEmit \|\| echo …`; with no `typecheck` script the final `echo` **always succeeded**, so tsc errors never failed CI (e.g. an incomplete test fixture shipped in #58). Added `"typecheck": "tsc --noEmit"` to `package.json`; tsc is clean repo-wide. | PR-networth-history |
| NEW · recurring-transaction generator | ✅ done — `getRecurringBills` now returns `displayVendor` (original casing) + `loggedThisMonth`. `RecurringBillsInsight` gains a **"Log this month"** button per detected bill not yet logged this month — `add_record` kind `expense` at the vendor's usual (LKR) amount, `recurring: true`, a note marking its origin; shows "✓ logged this month" once covered. | PR-recurring-generator |
| U4 · grab-bag Records tab / raw DataTables | ✅ **done** — all three raw `<DataTable>`s gone. Budgets → inline edit/delete in `BudgetPanel` (#61). Savings goals → new **`SavingsGoalsPanel`** (full CRUD over every field: name / target / current / currency / date / monthly / priority / status / goalKind); `SavingsGoalsProgress` stays the Overview progress view, `SinkingFundsPanel` the schedule tracker. Tax records → new **`TaxRecordsPanel`** (full CRUD, all 10 former columns). `DataTable` import removed from the screen. | #61 + PR-records-panels |
| NEW · category-level budget alerts | ✅ done — `financeAlerts` now emits per-category budget alerts: **critical** when over budget, **warning** at ≥90% used *while ≥3 days remain in the month* (so it doesn't nag late). Surfaces on the dashboard alerts card; `personal-finance-digest.sh` gains a "🟡 Nearly spent (≥90%)" section alongside the existing over-budget list. | PR-budget-alerts |
| NEW · scheduled / future-dated transactions | ✅ done — `scheduled_transactions` collection (LKR, `pending`/`posted`/`cancelled`, PG-wired + guard-tested) + CRUD kind `scheduled_transaction`. `post_scheduled` action turns a pending row into a real income/expense record (`addFinanceRecord` + mark `posted` + `postedRecordId`). `getUpcomingMoney.scheduled` lists pending rows in a −14..+45 day window. UI: `ScheduledTransactionsPanel` on Records (add / edit / cancel / Post now) + a "Coming up" section in `UpcomingMoney` with **Post now**. | PR-scheduled-txns |
| NEW · net-worth chart cash/investments/debt breakdown | ✅ done — snapshots already store `cashLkr`/`investmentsLkr`/`debtLkr`; `netWorthHistory` now carries them (`*Base`). `NetWorthHistoryCard` gets a Total ⇄ Breakdown toggle — breakdown draws three lines (cash / investments / debt) with a legend. | PR-networth-breakdown |
| NEW · recurring-bill drift warning + "Log all" | ✅ done — `getRecurringBills` returns `thisMonthAmount` + `drift` (fractional vs the vendor's usual). `RecurringBillsInsight` shows "▲ 20% vs usual this month" when `|drift| ≥ 15%`, and a "Log all N for this month" button when more than one bill is un-logged (posts each sequentially). | PR-recurring-drift |
| 12c · transfer entry + edit form | ✅ done — add-form "Transfer" mode (#48) + dedicated inline edit form (#49); `convertedLkrAmount` derived server-side on write (#52). | #48/#49/#52 |
| 2 · transaction splits (one expense → many categories) | ✅ done — optional `ExpenseRecord.splits[]` (`{category, amount, subcategory?, notes?}`); parts must sum to `amount`, enforced on add **and** update; `[]`/`null` clears. `expenseCategoryBreakdown(exp)` gives the pro-rata LKR-per-category view and is the single point the category consumers go through — `getBudgetVsActual`, `getFinanceTrends.categoriesThisMonth`, `buildFinanceQueryContext.byCategory`. **Record-level totals** (`financeSummary`, `getMonthlySummary`, trend series) are untouched — they read `convertedLkrAmount`, which the sum invariant keeps correct. UI: a `SplitsField` editor (rows + running balance) on the expense add **and** edit forms; split rows show inline. | PR-followups-expense-splits |
| 2a · split by percentage | ✅ done — each `SplitsField` row has a write-through `%` input (`amount = total * pct/100`) and a "Balance last row" button so 33/33/34-style splits still reconcile; `splitRowsFromPercents()` puts the rounding drift on the last row. | PR-splits-percentage |
| 2b · remembered split per merchant | ✅ done — `Merchant.defaultSplits?: Array<{category, percent}>` (percentages, positive, sum ~100, enforced on write; `[]` clears). "Save as vendor default" on the split editor persists the current split; typing a known vendor on a new expense pre-fills it scaled to the amount (reuses the exact-name blur seam). Shown read-only on the merchant row. | PR-splits-percentage |
