...
| U4 · grab-bag Records tab / raw DataTables (budgets slice) | ✅ partial — `BudgetPanel` gained inline **edit + delete** for every budget category ("All budgets" list); the redundant raw "Budget categories" `<DataTable>` is removed from Records, replaced by a one-line pointer to the Income tab. **Deferred**: the "Savings goals" and "Tax records" `<DataTable>`s — Savings goals is edited in two places (rich card + table) and Tax records has no purpose-built panel; both need a new panel + a field-coverage audit (editing-surface regression risk), out of scope for this pass. | PR-budget-editor |
