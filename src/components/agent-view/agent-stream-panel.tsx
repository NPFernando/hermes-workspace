        ...
        <div className="flex h-full min-h-0 flex-col">
          <div className="sticky top-0 z-10 border-b border-[var(--theme-border)] backdrop-blur glass-dropdown">
            ...
            <div className="flex items-start justify-between gap-3 px-4 py-3">
              <div className="min-w-0">
                <div className="flex items-center gap-2">
                  <span
                    className={`h-2.5 w-2.5 rounded-full ${AGENT_COLOR_DOT_CLASS[agentColor] ?? 'bg-[var(--theme-muted)]'}`}
                  />
              ...
              <button
                type="button"
                onClick={onClose}
                className="rounded-lg px-2 py-1 text-sm text-[var(--theme-muted)] hover:bg-[var(--theme-card2)]"
            ...
            <div className="flex flex-wrap gap-2 px-4 pb-3">
              <span className="inline-flex items-center gap-1 rounded-lg bg-[var(--theme-card)] px-2 py-1 text-xs">
              ...
              <span className="inline-flex items-center gap-1 rounded-lg bg-[var(--theme-card)] px-2 py-1 text-xs">
                <span className="text-[var(--theme-muted)]">Cost</span>
                <span className="font-medium text-[var(--theme-text)]">
                  {formatCost(totalCost)}
                </span>
              </span>
              <span className="inline-flex items-center gap-1 rounded-lg bg-[var(--theme-card)] px-2 py-1 text-xs">
                <span className="text-[var(--theme-muted)]">Runtime</span>
                <span className="font-medium text-[var(--theme-text)]">
            ...
            {messages.length === 0 ? (
              ...
              messages.map((message) => (
                ...
                >
                  ...
                  <p
                    className={`whitespace-pre-wrap text-xs text-[var(--theme-text)] ${message.role === 'tool' ? 'font-mono' : ''}`}
                  >
                    {message.text}
                  </p>
                </div>
          ...
          <div className="sticky bottom-0 border-t border-[var(--theme-border)] px-3 py-3 backdrop-blur glass-dropdown">
            <div className="grid grid-cols-3 gap-2">
              <button
                ...
                className="rounded-lg border border-[var(--theme-border)] px-2 py-2 text-xs font-medium text-[var(--theme-text)] hover:bg-[var(--theme-card2)]"
              >
                Steer
              </button>
              <div className="relative">
                {menuOpen ? (
                  <div className="absolute bottom-full left-0 mb-2 w-full rounded-lg border border-[var(--theme-border)] bg-[var(--theme-panel)] p-1 shadow-lg">
                    <button
                      type="button"
                      onClick={() => void onPauseToggle()}
                      disabled={pausePending}
                      className="flex w-full rounded-md px-2 py-1.5 text-left text-xs text-[var(--theme-text)] hover:bg-[var(--theme-card2)] disabled:opacity-60"
                    >
                      {pausePending
                        ? 'Updating...'
                        : isPaused
                ...
                <button
                  ...
                  className="w-full rounded-lg border border-[var(--theme-border)] px-2 py-2 text-xs font-medium text-[var(--theme-text)] hover:bg-[var(--theme-card2)]"
                >
                  Pause/Kill
                </button>
              </div>
