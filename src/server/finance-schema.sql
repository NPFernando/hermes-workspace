...
-- Runtime now keeps the durable Finance snapshot in Postgres when HERMES_PG_*
-- credentials are available, with ~/.hermes/finance/finance.json retained as
-- a local fallback/backup. The relational tables below remain the queryable
-- production shape for phased table-by-table migration.
...
CREATE TABLE IF NOT EXISTS savings_goals (
  ...
  progress_percentage REAL GENERATED ALWAYS AS (CASE WHEN target_amount > 0 THEN current_amount * 100.0 / target_amount ELSE 0 END) STORED,
...
CREATE TABLE IF NOT EXISTS trade_executions (id TEXT PRIMARY KEY, order_id TEXT REFERENCES trade_orders(id), plan_id TEXT, platform TEXT NOT NULL, symbol TEXT NOT NULL, side TEXT NOT NULL, quantity REAL NOT NULL, price REAL NOT NULL, fees REAL NOT NULL DEFAULT 0, executed_at TEXT NOT NULL, source TEXT NOT NULL, created_at TEXT NOT NULL, updated_at TEXT NOT NULL);
...
CREATE TABLE IF NOT EXISTS strategy_results (id TEXT PRIMARY KEY, strategy_id TEXT NOT NULL, win_rate REAL, loss_rate REAL, average_profit REAL, average_loss REAL, profit_factor REAL, max_drawdown REAL, confidence REAL, disabled INTEGER NOT NULL DEFAULT 0, source TEXT NOT NULL, created_at TEXT NOT NULL, updated_at TEXT NOT NULL, extra JSONB);
...
CREATE TABLE IF NOT EXISTS composite_sentiment_scores (
  id TEXT PRIMARY KEY,
