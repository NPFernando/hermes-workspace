...
describe('parseLlmResponse', () => {
  it('parses a valid decision', () => {
    const d = parseLlmResponse(
      '{"symbol":"BTCUSDT","signal":"BUY","confidence":0.8,"reasoning":"trend up"}',
    )
  ...
  it('clamps confidence into [0, 1]', () => {
    expect(
      parseLlmResponse('{"signal":"BUY","confidence":5,"reasoning":""}')
        ?.confidence,
    ).toBe(1)
    expect(
      parseLlmResponse('{"signal":"BUY","confidence":-2,"reasoning":""}')
        ?.confidence,
    ).toBe(0)
  })
...
describe('runLlmSignalCycle gating', () => {
  ...
  it('serializes overlapping cycles — the second is rejected as busy', async () => {
    ...
    const slowCallModel = async () =>
      new Promise<{ content: string; model: string } | null>((resolve) =>
        setTimeout(
          () =>
            resolve({
              content: '{"signal":"HOLD","confidence":0.5,"reasoning":"x"}',
              model: 'test',
            }),
          20,
        ),
    ...
    const busy = [a, b].filter(
      (r) => !r.ran && r.reason === 'already in progress',
    )
  ...
  it('opens a real position on a high-confidence BUY decision', async () => {
    await setMode('testnet_execute')
    await enableEngine()
    const { runLlmSignalCycle, getLlmSignalState } =
      await import('./llm-signal-engine')
    const fakeClient = {
      host: 'demo-api.binance.com',
      environment: 'testnet' as const,
      ping: async () => true,
    ...
    const callModel = async () => ({
      content:
        '{"symbol":"BTCUSDT","signal":"BUY","confidence":0.9,"reasoning":"strong trend"}',
      model: 'test-model',
    })
    const result = await runLlmSignalCycle({
      client: fakeClient as never,
  ...
  it('ignores a decision below the configured minimum confidence', async () => {
    ...
    const callModel = async () => ({
      content:
        '{"symbol":"BTCUSDT","signal":"BUY","confidence":0.1,"reasoning":"weak"}',
      model: 'test-model',
    })
    const result = await runLlmSignalCycle({
      client: fakeClient as never,
