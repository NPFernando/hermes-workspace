...
afterEach(() => {
  ...
  if (originalConfigMode === undefined) delete process.env.HERMES_SHARED_BUDGET_CONFIG
...
function provider(used: number): ProviderUsageResult {
  return {
    ...
    status: 'ok',
    updatedAt: Date.now(),
    lines: [{ type: 'text', label: 'Daily key spend', measure: 'spend', value: `$${used.toFixed(2)}` }],
  }
}
...
describe('shared usage budget', () => {
  it('is explicit when no budget policy is configured', () => {
    ...
    expect(buildSharedUsageBudget([provider(1)])).toMatchObject({ level: 'unconfigured', usedUsd: null })
  })

  it('aggregates observed daily spend and emits threshold levels', () => {
    process.env.HERMES_SHARED_DAILY_BUDGET_USD = '10'
    expect(buildSharedUsageBudget([provider(4), provider(4)])).toMatchObject({ level: 'warning', usedUsd: 8, remainingUsd: 2, percentUsed: 80 })
    expect(buildSharedUsageBudget([provider(10)])).toMatchObject({ level: 'exhausted', percentUsed: 100 })
  })

  it('does not convert missing provider readings into zero data', () => {
    process.env.HERMES_SHARED_DAILY_BUDGET_USD = '10'
    expect(buildSharedUsageBudget([{ ...provider(0), lines: [] }])).toMatchObject({ level: 'no_data', usedUsd: null })
  })
})
