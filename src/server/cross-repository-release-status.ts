...
export const DEFAULT_RELEASE_REPOSITORIES: Array<ReleaseRepository> = [
  { id: 'astrology', label: 'Astrology', slug: 'NPFernando/fernandofamily-astrology' },
  { id: 'hermes', label: 'Hermes Workspace', slug: 'NPFernando/hermes-workspace' },
  { id: 'harp', label: 'Universal HARP', slug: 'NPFernando/harp-control-plane' },
  { id: 'workspace-manager', label: 'Workspace Manager', slug: 'NPFernando/workspace-session-manager' },
]

type CommandRunner = (args: Array<string>) => Promise<{ stdout: string; stderr: string }>

async function runGh(args: Array<string>): Promise<{ stdout: string; stderr: string }> {
...
function configuredRepositories(env: NodeJS.ProcessEnv = process.env): Array<ReleaseRepository> {
  const configured = env.HERMES_RELEASE_REPOSITORIES
    ?.split(',')
    .map((slug) => slug.trim())
    .filter(Boolean)
  ...
  return configured.map((slug) => {
    ...
    return { id: name.toLowerCase().replace(/[^a-z0-9]+/g, '-'), label: name, slug }
  })
}

function unavailable(repository: ReleaseRepository, detail: string): RepositoryReleaseStatus {
  return {
    ...repository,
    status: 'unavailable',
    detail,
...
export async function getCrossRepositoryReleaseStatus({
  ...
  run?: CommandRunner
} = {}): Promise<{ generatedAt: string; repositories: Array<RepositoryReleaseStatus> }> {
  const results = await Promise.all(repositories.map(async (repository) => {
    try {
      const [metadataResult, runsResult] = await Promise.all([
        run(['repo', 'view', repository.slug, '--json', 'defaultBranchRef,pushedAt']),
        run(['run', 'list', '--repo', repository.slug, '--limit', '1', '--json', 'status,conclusion,headSha,workflowName,createdAt']),
      ])
      ...
      return unavailable(repository, error instanceof Error ? error.message : 'GitHub evidence unavailable.')
    }
  }))
