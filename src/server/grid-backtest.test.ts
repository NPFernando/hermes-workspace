...
describe('runGridBacktest', () => {
  ...
  it('never fills a level the price never touches', () => {
    ...
    expect(report.trades.filter((t) => t.reason === 'grid-fill')).toHaveLength(
      0,
    )
  })
  ...
  it('liquidates on a stop breach and halts new entries without autoRecenter', () => {
    ...
    for (let i = 7; i < 12; i++)
      candles.push(candle(i, { open: 92, high: 95, low: 85, close: 90 }))
    ...
    const stopTrades = report.trades.filter(
      (t) => t.reason === 'stop-liquidation',
    )
  ...
  it('re-arms after a stop breach when autoRecenter is set', () => {
    ...
    for (let i = 7; i < 12; i++)
      candles.push(candle(i, { open: 118, high: 122, low: 114, close: 118 }))
    ...
    expect(report.trades.some((t) => t.reason === 'stop-liquidation')).toBe(
      true,
    )
  ...
  it('engages the absolute floor once a re-arm has recentered below it, and halts for good', () => {
    ...
    const floorTrades = report.trades.filter(
  ...
  it('pauses entries once the chop gate detects a trend, and resumes after', () => {
    ...
    for (let i = 0; i < 5; i++)
      candles.push(candle(i, { open: 100, high: 101, low: 99, close: 100 }))
    ...
    for (let i = 10; i < 13; i++)
      candles.push(candle(i, { open: 99, high: 140, low: 60, close: 99 }))
  ...
  it('re-arms after price idles outside the range without tripping the stops', () => {
    ...
    expect(
      control.trades.filter((t) => t.reason === 'range-idle-rearm'),
    ).toHaveLength(0)
    expect(control.trades.filter((t) => t.reason === 'grid-fill')).toHaveLength(
      0,
    )
  })
  ...
  it('reports a per-symbol buy-and-hold return for comparison', () => {
    ...
    const report = runGridBacktest(
      { BTCUSDT: candles },
      cfg({ rangeLookbackCandles: 5, gridCount: 3 }),
    )
