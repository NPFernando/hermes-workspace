...
type CalibrationAccumulator = Omit<
  PaperDecisionCalibrationBucket,
  'directionalHitRate'
>

const CALIBRATION_BUCKETS: Array<
  Pick<
    PaperDecisionCalibrationBucket,
    'label' | 'minScoreMagnitude' | 'maxScoreMagnitude'
  >
> = [
...
export function evaluatePaperDecisionQuality(input: {
  ...
  const maxOutcomeStalenessMs =
    input.maxOutcomeStalenessMs ?? Math.min(MAX_OUTCOME_STALENESS_MS, horizonMs)
  if (!Number.isFinite(horizonMs) || horizonMs <= 0)
    throw new Error('horizonMs must be positive')
  if (
    !Number.isFinite(maxOutcomeStalenessMs) ||
    maxOutcomeStalenessMs < 0 ||
    maxOutcomeStalenessMs > horizonMs
  )
    throw new Error('maxOutcomeStalenessMs must be between zero and horizonMs')
  ...
  for (const candles of candlesBySymbol.values())
    candles.sort(
      (a, b) => a.openedAtMs - b.openedAtMs || a.closedAtMs - b.closedAtMs,
    )
  ...
  for (const decision of input.decisions) {
    ...
    const outcomeCandles = (candlesBySymbol.get(symbol) ?? []).filter(
      (candle) =>
        candle.openedAtMs > decisionAtMs && candle.closedAtMs <= deadlineMs,
    )
    ...
    const signedReturnPct =
      ((finalCandle.close - entryPrice) / entryPrice) * 100 * direction
    const worstPrice =
      direction > 0
  ...
  return {
    ...
    averageAdverseMovePct:
      adverseMoves.length > 0
        ? round(
            adverseMoves.reduce((sum, value) => sum + value, 0) /
              adverseMoves.length,
          )
        : null,
    worstAdverseMovePct:
      adverseMoves.length > 0 ? round(Math.max(...adverseMoves)) : null,
    horizon: {
      durationMs: horizonMs,
      maxOutcomeStalenessMs,
