...
import type { UnlockModalPrereq } from './panels/unlock-prereq-modal'
import type { ArchivePreview } from './panels/archive-wizard-modal'
...
export function TasksScreen() {
  ...
  const [activePanel, setActivePanel] = useState<
    'activity' | 'tags' | 'sisterLoad' | 'rebalance' | 'blockers' | null
  >(null)
  ...
  const setShowActivity = (v: boolean | ((p: boolean) => boolean)) =>
    setActivePanel((prev) => {
  ...
  const setShowTagsPanel = (v: boolean | ((p: boolean) => boolean)) =>
    setActivePanel((prev) => {
      const cur = prev === 'tags'
      const next = typeof v === 'function' ? v(cur) : v
      if (next) return 'tags'
  ...
  const setShowSisterLoad = (v: boolean | ((p: boolean) => boolean)) =>
    setActivePanel((prev) => {
      const cur = prev === 'sisterLoad'
      const next = typeof v === 'function' ? v(cur) : v
      if (next) return 'sisterLoad'
      if (cur) return null
      return prev
    })
  const setShowRebalance = (v: boolean | ((p: boolean) => boolean)) =>
    setActivePanel((prev) => {
      const cur = prev === 'rebalance'
      const next = typeof v === 'function' ? v(cur) : v
      if (next) return 'rebalance'
      if (cur) return null
      return prev
    })
  const setShowBlockers = (v: boolean | ((p: boolean) => boolean)) =>
    setActivePanel((prev) => {
      const cur = prev === 'blockers'
      const next = typeof v === 'function' ? v(cur) : v
  ...
  const [denseMode, setDenseMode] = useState(
    () => localStorage.getItem('hermes-dense-mode') === '1',
  )
  ...
  const [notifLastSeen, setNotifLastSeen] = useState<string>(
    () =>
      localStorage.getItem('hermes-notif-seen') ?? new Date(0).toISOString(),
  )
  ...
  const [unlockModalPrereq, setUnlockModalPrereq] =
    useState<UnlockModalPrereq | null>(null)
  ...
  const taskRefreshStatus = formatTaskRefreshStatus(
  ...
  const queuePositions = useMemo(() => {
    ...
    const sorted = tasks
      .filter(
        (t) =>
          (t.column === 'todo' || t.column === 'backlog') && !t.agent_state,
      )
      .sort((a, b) => {
        ...
        const da = new Date(
          (a as unknown as { created_at?: string }).created_at ?? 0,
        ).getTime()
        const db = new Date(
          (b as unknown as { created_at?: string }).created_at ?? 0,
        ).getTime()
        return da - db
      })
  ...
  const swimlaneData = useMemo(() => {
    ...
    const SWIM_COLS: Array<TaskColumn> = [
      'todo',
      'in_progress',
      'review',
      'blocked',
  ...
  async function handleTaskClarify(
    taskId: string,
  ...
  const panelLive = panelTask
    ? (tasks.find((t) => t.id === panelTask.id) ?? panelTask)
    : null
  ...
  const timeoutAnalysisData = useMemo(() => {
    ...
    timedOutEntries.forEach((e) => {
      const a = e.task.assignee ?? 'unassigned'
      byAssignee[a] = (byAssignee[a] ?? 0) + 1
    })
    const byTag: Record<string, number> = {}
    timedOutEntries.forEach((e) => {
      const tag = e.task.tags.at(0) ?? 'untagged'
  ...
  return (
    ...
    >
          ...
          <div className="flex items-start justify-between gap-3">
            <div className="min-w-0 flex-1">
              <div className="flex items-center gap-2">
                <h1 className="hidden text-2xl font-medium text-ink shrink-0 md:block">
                  Tasks
                </h1>
                {assigneeFilter && (
                  <div className="flex items-center gap-1.5 text-xs text-[var(--theme-muted)]">
                    ...
                    <span>
                      Filtered by:{' '}
                      <span className="capitalize text-[var(--theme-warning)]">
                        {assigneeFilter}
                      </span>
                    </span>
                    <button
              ...
              <div className={TASK_STATS_ROW_CLASS}>
                ...
                {stats.blocked > 0 && (
                  <>
                    ...
                    <button
                      ...
                      aria-label={formatTaskFilterAriaLabel(
                        'Blocked',
                        filterBlocked,
                      )}
                      className="text-[var(--theme-danger)] cursor-pointer hover:opacity-80 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[color-mix(in_srgb,var(--theme-danger)_60%,transparent)] rounded-sm"
                      onClick={() => setFilterBlocked((v) => !v)}
                      title={formatBlockedTaskBreakdownTitle(
                        stats.blockedWaiting,
                        stats.blockedExecFail,
                      )}
                    >
                ...
                {stats.timedOut > 0 && (
                  <>
                    ...
                    <button
                      ...
                      className="text-[var(--theme-warning)] hover:text-[var(--theme-warning)] focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[color-mix(in_srgb,var(--theme-warning)_60%,transparent)] rounded-sm transition-colors"
                    >
                ...
                {(() => {
                  const doneToday =
                    ...
                    )?.count ?? (sweepStats ? sweepStats.completed : 0)
                  if (goalInputOpen)
                    return (
                  ...
                  if (dailyGoal == null)
                    return (
                      <button
                        ...
                        className="text-[10px] opacity-40 hover:opacity-80 hover:text-[var(--theme-accent)] transition-colors"
                      >
                        + goal
                      </button>
                    )
                  ...
                  return (
                    <button
                      type="button"
                      onClick={() => {
                        setGoalInputVal(String(dailyGoal))
                        setGoalInputOpen(true)
                      }}
                      className="flex items-center gap-1"
                      title={`${doneToday}/${dailyGoal} today — click to update`}
                    >
                ...
                {stats.agentActive > 0 && (
                  <>
                    <span className="opacity-30">·</span>
                    <span className="flex items-center gap-1 text-[var(--theme-accent-secondary)]">
                      <span className="w-1 h-1 rounded-full bg-[var(--theme-accent-secondary)] animate-pulse inline-block" />
                      Astra {stats.agentActive}
                    </span>
                  </>
                )}
                {/* Pipeline inline */}
                {sweepStats && sweepStats.dispatched > 0 && (
                  <>
                    ...
                    <span className="opacity-60">
                      🔄 {sweepStats.dispatched}
                    </span>
                    {sweepStats.completed > 0 && (
                      <span className="text-[var(--theme-success)]">
                        ✅ {sweepStats.completed}
                      </span>
                    )}
                    {sweepStats.successRate !== null && (
                      <span
                        className={
                          sweepStats.successRate >= 50
                            ? 'text-[var(--theme-success)]'
                            : sweepStats.successRate >= 25
                              ? 'text-[var(--theme-warning)]'
                              : 'text-[var(--theme-danger)]'
                        }
                      >
                        {sweepStats.successRate}%
                      </span>
                    )}
                    {sweepStats.lastSweepAt && (
                      <span className="opacity-40 text-[10px]">
                        sweep{' '}
                        {Math.round(
                          (Date.now() -
                            new Date(sweepStats.lastSweepAt).getTime()) /
                            60_000,
                        )}
                        m ago
                      </span>
                      ...
                      (() => {
                        ...
                        return (
                          <span
                            ...
                            title={`7d: ${trend.total} completed`}
                          >
                            <svg width={W} height={H} viewBox={`0 0 ${W} ${H}`}>
                              {trend.trend.map((d, i) => {
                                const h = Math.max(
                                  1,
                                  Math.round((d.count / max) * H),
                                )
                                return (
                                  <rect
                                    key={d.date}
                  ...
                  <>
                    ...
                    <div className="relative">
                      <button
                        ...
                        className="text-[var(--theme-success)] hover:text-[var(--theme-success)] transition-colors"
                      >
                        🏆 {stats.todayWins.length}
                        {showWins ? ' ▲' : ''}
                      </button>
                      {showWins && (
                        <>
                          ...
                          />
                          <div className="absolute left-0 top-full mt-1 z-40 w-72 bg-[var(--theme-card)] border border-[color-mix(in_srgb,var(--theme-success)_20%,transparent)] rounded-xl shadow-2xl p-2 flex flex-col gap-0.5 max-h-64 overflow-y-auto scrollbar-thin">
                            <p className="text-[9px] font-semibold text-[color-mix(in_srgb,var(--theme-success)_60%,transparent)] uppercase tracking-wider px-2 pb-1">
                              Today's wins
                            </p>
                            {stats.todayWins
                              .slice(0, 12)
                              .map(({ task, completedAt }) => {
                                const ms =
                                ...
                                return (
                                  <button
                                    key={task.id + completedAt}
                                    type="button"
                                    onClick={() => {
                                      setPanelTask(task)
                                      setShowWins(false)
                                    }}
                                    className="flex items-center justify-between gap-2 px-2 py-1.5 rounded-lg hover:bg-[color-mix(in_srgb,var(--theme-success)_10%,transparent)] transition-colors text-left w-full"
                                  >
                                    <span className="text-[11px] text-[var(--theme-success)] truncate">
                                      {task.title}
                                    </span>
                                    <span className="text-[9px] text-[color-mix(in_srgb,var(--theme-success)_60%,transparent)] shrink-0">
                                      {ago}
                                    </span>
                                  </button>
                                )
                              })}
                            {stats.todayWins.length > 12 && (
                              <p className="text-[9px] text-center text-[color-mix(in_srgb,var(--theme-success)_40%,transparent)] pb-1">
                                +{stats.todayWins.length - 12} more
                              </p>
                            )}
                          </div>
                        </>
            ...
            <div className="flex items-center gap-2 shrink-0 flex-wrap justify-end">
              ...
              <button
                ...
                onClick={() => {
                  if (!showActivity) {
                    markNotifSeen()
                    setActivityTab(
                      stats.inboxTasks.length > 0 ? 'inbox' : 'feed',
                    )
                  }
                  setShowActivity((v) => !v)
                }}
                className={cn(
                  ...
                  showActivity
                    ? 'border-[color-mix(in_srgb,var(--theme-warning)_50%,transparent)] bg-[color-mix(in_srgb,var(--theme-warning)_10%,transparent)] text-[var(--theme-warning)]'
                ...
                {activityCount > 0 && (
                  <span
                    className={cn(
                      ...
                      showActivity
                        ? 'bg-[color-mix(in_srgb,var(--theme-warning)_20%,transparent)] text-[var(--theme-warning)]'
              ...
              <button
                ...
                className={cn(
                  'relative flex items-center gap-1.5 rounded-lg px-2.5 py-1.5 text-xs font-medium border transition-colors',
                  showBlockers
                    ? 'border-[color-mix(in_srgb,var(--theme-danger)_50%,transparent)] bg-[color-mix(in_srgb,var(--theme-danger)_10%,transparent)] text-[var(--theme-danger)]'
                    : 'border-[var(--theme-border)] text-[var(--theme-muted)] hover:bg-[var(--theme-hover)]',
                )}
              ...
              <MenuRoot>
                ...
                <MenuContent side="bottom" align="end">
                  ...
                  <MenuItem
                    onClick={() =>
                      setViewMode((v) => (v === 'board' ? 'swimlane' : 'board'))
                    }
                  >
              ...
              <button
                ...
                className={cn(
                  'flex items-center gap-1.5 rounded-lg px-2.5 py-1.5 text-xs font-medium border transition-colors',
                  askingAstra
                    ? 'border-[color-mix(in_srgb,var(--theme-accent-secondary)_50%,transparent)] bg-[color-mix(in_srgb,var(--theme-accent-secondary)_10%,transparent)] text-[var(--theme-accent-secondary)] cursor-wait'
                    : 'border-[var(--theme-border)] text-[var(--theme-accent-secondary)] hover:bg-[color-mix(in_srgb,var(--theme-accent-secondary)_10%,transparent)] hover:border-[color-mix(in_srgb,var(--theme-accent-secondary)_50%,transparent)]',
                )}
              >
              ...
              {(() => {
                const totalActions =
                  ...
                  stats.stubReviewCount +
                  stats.timedOut +
                  stats.gatedPrereqs.reduce((s, g) => s + g.count, 0)
                const anyBusy =
                  batchExecuting ||
                  drainingNow ||
                  replanningStubs ||
                  rescuingTimedOut ||
                  unlockingPrereq != null
                return (
                  <MenuRoot>
                    <MenuTrigger
                      render={
                        <button
                          ...
                          className={cn(
                            'flex items-center gap-1.5 rounded-lg px-2.5 py-1.5 text-xs font-medium border transition-colors',
                            totalActions > 0
                              ? 'border-[color-mix(in_srgb,var(--theme-success)_40%,transparent)] text-[var(--theme-success)] hover:bg-[color-mix(in_srgb,var(--theme-success)_10%,transparent)]'
                              : 'border-[var(--theme-border)] text-[var(--theme-muted)] hover:bg-[var(--theme-hover)]',
                        ...
                        >
                          <HugeiconsIcon
                            ...
                            size={13}
                            strokeWidth={1.8}
                            className={anyBusy ? 'animate-pulse' : ''}
                          />
                          {anyBusy ? 'Working…' : 'Actions'}
                          {totalActions > 0 && (
                            <span className="text-[9px] px-1 rounded-full bg-[color-mix(in_srgb,var(--theme-success)_20%,transparent)] text-[var(--theme-success)] font-bold">
                              {totalActions}
                            </span>
                          )}
                        </button>
                    ...
                    <MenuContent side="bottom" align="end">
                        ...
                        <MenuItem
                          onClick={async () => {
                            ...
                            try {
                              ...
                              toast(
                                res.remaining > 0
                                  ? `Started ${res.started} · ${res.remaining} more ready`
                                  : `Started ${res.started} task${res.started !== 1 ? 's' : ''}`,
                              )
                            } catch {
                              toast('Execute failed', { type: 'error' })
                            } finally {
                      ...
                      {stats.readyToExecute > 0 && (
                        <MenuItem
                          onClick={async () => {
                            setDrainingNow(true)
                            try {
                              const res = await fetch('/api/tasks-drain-now', {
                                method: 'POST',
                              ...
                              toast(
                                data.queued > 0
                                  ? `Drained ${data.queued} to execution`
                                  : 'No eligible tasks right now',
                              )
                            } catch {
                              toast('Drain failed', { type: 'error' })
                            } finally {
                        ...
                        <MenuItem
                          onClick={async () => {
                            ...
                            try {
                              ...
                              toast('Re-plan failed', { type: 'error' })
                            } finally {
                              setReplanningStubs(false)
                            }
                          }}
                        ...
                        <MenuItem
                          onClick={async () => {
                            ...
                            try {
                              ...
                              toast(
                                data.rescued > 0
                                  ? `Rescued ${data.rescued} stuck tasks`
                                  : 'No stuck tasks',
                              )
                            } catch {
                              toast('Rescue failed', { type: 'error' })
                            } finally {
                              setRescuingTimedOut(false)
                            }
                          }}
                      ...
                      {stats.gatedPrereqs.map(({ id, count, title }) => (
                        <MenuItem
                          key={id}
                          onClick={() =>
                            setUnlockModalPrereq({ id, count, title })
                          }
                      ...
                      {totalActions === 0 && (
                        <MenuItem
                          disabled
                          className="text-[var(--theme-muted)] opacity-50 cursor-default"
                        >
                          No pending actions
                        </MenuItem>
                      )}
                ...
                <MenuTrigger
                  render={
                    ...
                    >
                      ...
                      {(tasks.some(
                        (t) =>
                          t.column === 'review' || t.column === 'in_progress',
                      ) ||
                        tasks.some(
                          (t) =>
                            (t.column === 'backlog' || t.column === 'todo') &&
                            !t.agent_history?.length,
                        ) ||
                        columnMap['done'].length > 0 ||
                        tasks.some(isStuckAgent)) && (
                        <span className="absolute -top-0.5 -right-0.5 w-2 h-2 rounded-full bg-[var(--theme-warning)] border-2 border-[var(--theme-panel)]" />
                      )}
                    </button>
                  }
                />
                <MenuContent side="bottom" align="end">
                  ...
                  {tasks.some(
                    (t) =>
                      t.agent_state &&
                      t.agent_action_at &&
                      Date.now() - new Date(t.agent_action_at).getTime() >
                        10 * 60_000,
                  ) && (
                    <MenuItem
                      onClick={async () => {
                        ...
                        try {
                          await fetch('/api/tasks-deploy-agents', {
                            method: 'DELETE',
                          })
                          await tasksQuery.refetch()
                        } finally {
                    ...
                    >
                      <HugeiconsIcon
                        icon={Loading03Icon}
                        size={13}
                        className={clearingStuck ? 'animate-spin' : ''}
                      />
                  ...
                  {/* Check Completion */}
                  {tasks.some(
                    (t) => t.column === 'review' || t.column === 'in_progress',
                  ) && (
                    <MenuItem
                      onClick={async () => {
                        setCheckingCompletion(true)
                        try {
                          await fetch('/api/tasks-completion-check', {
                            method: 'POST',
                          })
                          await tasksQuery.refetch()
                        } finally {
                    ...
                    >
                      <HugeiconsIcon
                        icon={CheckListIcon}
                        size={13}
                        className={checkingCompletion ? 'animate-pulse' : ''}
                      />
                  ...
                  <MenuItem
                    onClick={async () => {
                      ...
                      try {
                        ...
                        toast('Failed to reach idea generator', {
                          type: 'error',
                        })
                      } finally {
                  ...
                  {tasks.some(
                    ...
                    >
                      <HugeiconsIcon
                        icon={Delete01Icon}
                        size={13}
                        className={pruningStale ? 'animate-pulse' : ''}
                      />
              ...
              >
                <HugeiconsIcon
                  ...
                  className={cn(
                    ...
                    tasksQuery.isFetching &&
                      'animate-spin text-[var(--theme-accent)]',
                  )}
                />
              </button>
        ...
        <div className="flex flex-wrap items-center gap-2 relative">
          ...
          <div className="relative">
            <button
              ...
              className={cn(
                'rounded-full w-5 h-5 flex items-center justify-center text-[10px] border transition-colors',
                showSearchHelp
                  ? 'border-[var(--theme-accent)] text-[var(--theme-accent)] bg-[var(--theme-accent)]/10'
                  : 'border-[var(--theme-border)] text-[var(--theme-muted)] hover:border-[var(--theme-accent)] hover:text-[var(--theme-accent)]',
              )}
            ...
            {showSearchHelp && (
              <>
                ...
                <div className="absolute left-0 top-full mt-1 z-40 w-64 bg-[var(--theme-card)] border border-[var(--theme-border)] rounded-xl shadow-2xl p-3 flex flex-col gap-1.5">
                  <p className="text-[9px] font-semibold text-[var(--theme-muted)] uppercase tracking-wider pb-1 border-b border-[var(--theme-border)]">
                    Search syntax
                  </p>
                  {(
                    [
                    ...
                    >
                      <code className="text-[10px] text-[var(--theme-accent)] font-mono">
                        {syntax}
                      </code>
                  ...
                  <p className="text-[9px] text-[var(--theme-muted)] opacity-50 pt-1 border-t border-[var(--theme-border)]">
                    Combine freely:{' '}
                    <code className="text-[9px] text-[var(--theme-accent)]">
                      assignee:ada is:blocked
                    </code>
                  </p>
                </div>
              </>
          ...
          <div className="relative">
            <button
              ...
              aria-label={
                showFilterPopover ? 'Close task filters' : 'Open task filters'
              }
              className={cn(
                ...
                <div className="absolute left-0 top-full mt-1 z-40 w-72 bg-[var(--theme-card)] border border-[var(--theme-border)] rounded-xl shadow-2xl p-3 flex flex-col gap-3">
                      ...
                      {(
                        [
                          ...
                          [
                            'Blocked',
                            filterBlocked,
                            () => setFilterBlocked((v) => !v),
                          ],
                          [
                            'Timed Out',
                            filterTimedOut,
                            () => setFilterTimedOut((v) => !v),
                          ],
                          [
                            'Active Agent',
                            filterActiveAgent,
                  ...
                  <div>
                    ...
                    <div className="flex gap-1.5">
                      {(['high', 'medium', 'low'] as Array<TaskPriority>).map(
                        (p) => (
                          <button
                            key={p}
                            type="button"
                  ...
                  <div>
                    ...
                    <div className="flex gap-1.5">
                      {(
                        ...
                        >
                      ).map(([band, label, accessibleLabel, color]) => (
                        <button
                          key={band}
                          type="button"
                          onClick={() =>
                          ...
                          style={{
                            borderColor:
                              ageFilter === band
                                ...
                                : 'var(--theme-border)',
                            color:
                              ageFilter === band ? color : 'var(--theme-muted)',
                            background:
                              ageFilter === band
                      ...
                      <div className="flex flex-wrap gap-1.5">
                        {stats.sisterChips.map(([name, count]) => (
                          <button
                            ...
                            aria-pressed={assigneeFilter === name}
                            aria-label={formatTaskFilterAriaLabel(
                              `${name} assignee`,
                              assigneeFilter === name,
                            )}
                        ...
                        {filterPresets.map((p) => (
                          ...
                          >
                            <button
                              ...
                              className="text-[10px] px-2 py-0.5 rounded-l-full border border-r-0 border-[var(--theme-border)] text-[var(--theme-muted)] hover:bg-[var(--theme-hover)]"
                            >
                              {p.name}
                            </button>
                            <button
                              ...
                              className="text-[10px] px-1.5 py-0.5 rounded-r-full border border-[var(--theme-border)] text-[var(--theme-muted)] hover:text-[var(--theme-danger)] hover:border-[color-mix(in_srgb,var(--theme-danger)_30%,transparent)]"
                            >
                              ×
                            </button>
                          </div>
                  ...
                  <div className="flex items-center justify-between pt-1 border-t border-[var(--theme-border)]">
                    <span className="text-[10px] text-[var(--theme-muted)] opacity-60">
                      {tasksByColumn.hasAnyFilter
                        ? formatTaskFilterSummary(
                            tasksByColumn.matchCount,
                            tasksByColumn.totalTasks,
                          )
                        : 'No active filters'}
                    </span>
                    <div className="flex gap-2">
                      {presetNameOpen ? (
                        <input
                          ...
                          onBlur={() => {
                            if (!presetNameVal.trim()) setPresetNameOpen(false)
                          }}
                          placeholder="preset name…"
                          className="w-24 rounded border border-[color-mix(in_srgb,var(--theme-accent-secondary)_40%,transparent)] bg-[var(--theme-card)] text-[10px] px-1.5 py-0.5 focus:outline-none focus:border-[var(--theme-accent-secondary)] text-[var(--theme-accent-secondary)]"
                        />
                      ) : (
                        <button
                          type="button"
                          ...
                          className="text-[10px] text-[var(--theme-accent-secondary)] hover:underline"
                        >
                          ★ Save
                        </button>
                      )}
          ...
          {filterOverdue && (
            <button
              ...
              className="flex items-center gap-1 text-[10px] px-2 py-0.5 rounded-full border border-[var(--theme-accent)] text-[var(--theme-accent)] bg-[var(--theme-accent)]/10 hover:bg-[var(--theme-accent)]/20 transition-colors"
            >
              Overdue <HugeiconsIcon icon={Cancel01Icon} size={9} />
            </button>
          )}
          {filterBlocked && (
            <button
              ...
              className="flex items-center gap-1 text-[10px] px-2 py-0.5 rounded-full border border-[var(--theme-accent)] text-[var(--theme-accent)] bg-[var(--theme-accent)]/10 hover:bg-[var(--theme-accent)]/20 transition-colors"
            >
              Blocked <HugeiconsIcon icon={Cancel01Icon} size={9} />
            </button>
          )}
          {filterTimedOut && (
            <button
              type="button"
              onClick={() => setFilterTimedOut(false)}
              className="flex items-center gap-1 text-[10px] px-2 py-0.5 rounded-full border border-[var(--theme-accent)] text-[var(--theme-accent)] bg-[var(--theme-accent)]/10 hover:bg-[var(--theme-accent)]/20 transition-colors"
            >
              Timed Out <HugeiconsIcon icon={Cancel01Icon} size={9} />
            </button>
          )}
          {filterActiveAgent && (
            <button
              type="button"
              onClick={() => setFilterActiveAgent(false)}
              className="flex items-center gap-1 text-[10px] px-2 py-0.5 rounded-full border border-[var(--theme-accent)] text-[var(--theme-accent)] bg-[var(--theme-accent)]/10 hover:bg-[var(--theme-accent)]/20 transition-colors"
            >
              Active Agent <HugeiconsIcon icon={Cancel01Icon} size={9} />
            </button>
          )}
          {filterInReview && (
            <button
              type="button"
              onClick={() => setFilterInReview(false)}
              className="flex items-center gap-1 text-[10px] px-2 py-0.5 rounded-full border border-[var(--theme-accent)] text-[var(--theme-accent)] bg-[var(--theme-accent)]/10 hover:bg-[var(--theme-accent)]/20 transition-colors"
            >
              In Review <HugeiconsIcon icon={Cancel01Icon} size={9} />
            </button>
          )}
          {hideSubtasks && (
            <button
              type="button"
              onClick={() => setHideSubtasks(false)}
              className="flex items-center gap-1 text-[10px] px-2 py-0.5 rounded-full border border-[var(--theme-accent)] text-[var(--theme-accent)] bg-[var(--theme-accent)]/10 hover:bg-[var(--theme-accent)]/20 transition-colors"
            >
              Subtasks hidden <HugeiconsIcon icon={Cancel01Icon} size={9} />
            </button>
          )}
          {priorityFilter && (
            <button
              ...
              className="flex items-center gap-1 text-[10px] px-2 py-0.5 rounded-full border capitalize"
              style={{
                borderColor: PRIORITY_COLORS[priorityFilter],
                color: PRIORITY_COLORS[priorityFilter],
                background: `${PRIORITY_COLORS[priorityFilter]}1a`,
              }}
            >
              {priorityFilter} <HugeiconsIcon icon={Cancel01Icon} size={9} />
            </button>
          )}
          {assigneeFilter && (
            <button
              type="button"
              onClick={() => setAssigneeFilter(null)}
              className="flex items-center gap-1 text-[10px] px-2 py-0.5 rounded-full border border-[var(--theme-accent)] text-[var(--theme-accent)] bg-[var(--theme-accent)]/10 capitalize"
            >
              {assigneeFilter} <HugeiconsIcon icon={Cancel01Icon} size={9} />
            </button>
          )}
          {ageFilter && (
            <button
            ...
            >
              {ageFilter} <HugeiconsIcon icon={Cancel01Icon} size={9} />
            </button>
          )}
          {tagFilter && (
            <button
              ...
              className="flex items-center gap-1 text-[10px] px-2 py-0.5 rounded-full border border-[var(--theme-accent)] text-[var(--theme-accent)] bg-[var(--theme-accent)]/10"
            >
              #{tagFilter} <HugeiconsIcon icon={Cancel01Icon} size={9} />
            </button>
          )}
          {tasksByColumn.hasAnyFilter && (
            <span className="ml-auto text-[10px] text-[var(--theme-muted)] opacity-60 whitespace-nowrap">
              {formatTaskFilterSummary(
                tasksByColumn.matchCount,
                tasksByColumn.totalTasks,
              )}
          ...
          return (
            <div className="shrink-0 mx-4 sm:mx-6 lg:mx-8 mb-1 rounded-lg border border-[var(--theme-border)] bg-[var(--theme-card)] p-3">
              <div className="flex items-center justify-between mb-2">
                <p className="text-[10px] font-semibold text-[var(--theme-muted)] uppercase tracking-wider">
                  Sister Load
                </p>
                <button
                  type="button"
              ...
              <div className="flex flex-col gap-1.5">
                {stats.sisterChips.map(([name, count]) => (
                  <div key={name} className="flex items-center gap-2">
                    ...
                    <span className="text-[10px] text-[var(--theme-muted)] shrink-0 w-8 text-right">
                      {count}
                    </span>
                  </div>
                ))}
      ...
      {stats.workingTasks.length > 0 && (
        <div className="shrink-0 mx-4 sm:mx-6 lg:mx-8 mb-1 rounded-lg border border-[color-mix(in_srgb,var(--theme-accent-secondary)_30%,transparent)] bg-[color-mix(in_srgb,var(--theme-accent-secondary)_5%,transparent)] overflow-hidden">
          <button
            type="button"
            onClick={() => setShowRunningPanel((v) => !v)}
            className="w-full flex items-center gap-2 px-3 py-1.5 text-[10px] text-[var(--theme-accent-secondary)] hover:bg-[color-mix(in_srgb,var(--theme-accent-secondary)_10%,transparent)] transition-colors"
          >
            <span className="w-1.5 h-1.5 rounded-full bg-[var(--theme-accent-secondary)] animate-pulse" />
            <span className="font-medium">
              {stats.workingTasks.length} executing
            </span>
          ...
          {showRunningPanel && (
            <div className="divide-y divide-[color-mix(in_srgb,var(--theme-accent-secondary)_10%,transparent)] max-h-52 overflow-y-auto">
      ...
      {(() => {
        ...
        const sweepAgoMin = sweepStats.lastSweepAt
          ? Math.round(
              (Date.now() - new Date(sweepStats.lastSweepAt).getTime()) /
                60_000,
            )
          : null
        const stalled = sweepAgoMin !== null && sweepAgoMin > 60
        const lowSuccess =
          sweepStats.successRate !== null && sweepStats.successRate < 10
        ...
        return (
          <div
            className={cn(
              ...
              isRed
                ? 'bg-[color-mix(in_srgb,var(--theme-danger)_8%,transparent)] border-[color-mix(in_srgb,var(--theme-danger)_30%,transparent)] text-[var(--theme-danger)]'
                : 'bg-[color-mix(in_srgb,var(--theme-warning)_8%,transparent)] border-[color-mix(in_srgb,var(--theme-warning)_30%,transparent)] text-[var(--theme-warning)]',
            )}
          >
            <span>{isRed ? '🚨' : '⚠️'}</span>
            <span className="flex-1">
              ...
              {!stalled &&
                lowSuccess &&
                `Low success rate: ${sweepStats.successRate}% (${sweepStats.timedOutToday} timeouts today).`}
            </span>
            <button
              onClick={async () => {
                setDrainingNow(true)
      ...
      {viewMode === 'swimlane' && swimlaneData && (
        <div className="flex-1 min-h-0 overflow-auto px-4 pb-[calc(var(--tabbar-h,80px)+1rem)] pt-3 sm:px-6 lg:px-8">
          ...
          >
            <thead>
              <tr>
                <th className="py-2 pr-3 text-[10px] font-semibold text-[var(--theme-muted)] uppercase tracking-wider w-28">
                  Assignee
                </th>
                {swimlaneData.cols.map((c) => (
                  <th
                    key={c}
                    className="py-2 px-2 text-[10px] font-semibold uppercase tracking-wider"
                    style={{ color: COLUMN_COLORS[c] }}
                  >
                    {COLUMN_LABELS[c]}
                  </th>
                ))}
              ...
              {swimlaneData.rows.map(([asgn, cells]) => (
                ...
                >
                  <td className="py-2 pr-3 align-top">
                    <span className="text-[11px] font-medium text-[var(--theme-text)] truncate block max-w-[100px]">
                      {asgn}
                    </span>
                    <span className="text-[9px] text-[var(--theme-muted)] opacity-50">
                      {swimlaneData.cols.reduce(
                        (s, c) => s + cells[c].length,
                        0,
                      )}{' '}
                      tasks
                    </span>
                  ...
                  {swimlaneData.cols.map((c) => (
                    ...
                    >
                      <div className="flex flex-col gap-0.5 max-h-48 overflow-y-auto scrollbar-thin pr-1">
                        {cells[c].length === 0 ? (
                          <span className="text-[9px] text-[var(--theme-muted)] opacity-30 pl-1">
                            —
                          </span>
                        ) : (
                          cells[c].slice(0, 20).map((t) => (
                            ...
                            >
                              <span className="text-[10px] text-[var(--theme-text)] truncate flex-1">
                                {t.title.slice(0, 50)}
                              </span>
                              {t.agent_state && (
                                <span
                        ...
                        {cells[c].length > 20 && (
                          <span className="text-[9px] text-[var(--theme-muted)] opacity-50 pl-1">
                            +{cells[c].length - 20} more
                          </span>
                        )}
                      </div>
                    </td>
      ...
      {viewMode === 'board' && (
        ...
        >
          {visibleColumns.map((col) => {
            ...
            const compactAriaLabel = formatCompactTaskColumnAriaLabel(
              columnLabel,
              colTasks.length,
            )
            const compactActionLabel =
              formatCompactTaskColumnActionLabel(columnLabel)
            const isDragOver = dragOverColumn === col
            // Compact when empty and not being dragged over
            const isCompact =
              colTasks.length === 0 && !isDragOver && !tasksQuery.isLoading
                  ...
                  <>
                              ...
                              {col === 'review' &&
                                (() => {
                                  const readyCount = colTasks.filter((t) => {
                                    ...
                                    const note =
                                      (
                                        planned[planned.length - 1] as {
                                          note?: string
                                        }
                                      ).note ?? ''
                                    return (
                                      !note.includes('Plan unavailable') &&
                                      note.length >= 80
                                    )
                                  }).length
                                  return readyCount > 0 ? (
                                    <MenuItem
                                      onClick={async () => {
                                        ...
                                        setBatchExecuting(true)
                                        try {
                                          const r = await batchExecuteTasks(
                                            50,
                                            ids,
                                          )
                                          await tasksQuery.refetch()
                                          toast(
                                            `Started ${r.started} review task${r.started !== 1 ? 's' : ''}`,
                                          )
                                        } catch {
                                ...
                                <MenuItem
                                  onClick={async () => {
                                    ...
                                    try {
                                      ...
                                      await tasksQuery.refetch()
                                      toast(
                                        `Moved ${data.moved} stubs back to todo`,
                                      )
                                    } catch {
                                      toast('Re-plan failed', { type: 'error' })
                                    } finally {
                    ...
                    {tasksQuery.isError ? (
                      <div className="flex flex-col gap-2 p-2 flex-1 min-h-0">
                        <div className="flex flex-col items-center justify-center py-8 gap-2 text-[var(--theme-danger)]">
                    ...
                    ) : colTasks.length >= VIRTUAL_THRESHOLD ||
                      (col === 'todo' && groupByParent && groupedTodoRows) ? (
                      <VirtualTaskList
                        tasks={
                          col === 'review'
                            ? (() => {
                                const ready = colTasks.filter((t) => {
                                  if (t.agent_state) return false
                                  const pl = (t.agent_history ?? []).filter(
                                    (h: { action: string }) =>
                                      h.action === 'planned',
                                  )
                                  if (!pl.length) return false
                                  const note =
                                    (pl[pl.length - 1] as { note?: string })
                                      .note ?? ''
                                  return (
                                    !note.includes('Plan unavailable') &&
                                    note.length >= 80
                      ...
                      <div
                        className={cn(
                          'flex flex-col p-2 flex-1 min-h-0 overflow-y-auto scrollbar-thin',
                          denseMode ? 'gap-0.5' : 'gap-2',
                        )}
                      >
                              ...
                              <TaskCard
                                ...
                                onExecute={async () => {
                                  ...
                                  try {
                                    ...
                                    await tasksQuery.refetch()
                                    toast(
                                      `Agent is working on "${task.title}"…`,
                                    )
                                  } catch {
                                    toast('Failed to start agent execution', {
                                      type: 'error',
      ...
      {selectedIds.size > 0 && (
        <div className="fixed bottom-[calc(var(--tabbar-h,80px)+12px)] left-1/2 -translate-x-1/2 z-50 flex items-center gap-2 rounded-xl border border-[var(--theme-border)] bg-[var(--theme-panel)] px-3 py-2 shadow-2xl text-xs flex-wrap max-w-[92vw]">
          <span className="text-[var(--theme-muted)] mr-1">
                ...
                <MenuItem
                  ...
                  onClick={async () => {
                    for (const id of selectedIds)
                      await moveMutation.mutateAsync({ id, column: col })
                    clearSelection()
                    toast(
                      `Moved ${selectedIds.size} tasks to ${COLUMN_LABELS[col]}`,
                    )
                  }}
                >
                  {COLUMN_LABELS[col]}
          ...
          <button
            ...
            className="px-2.5 py-1 rounded-lg border border-[color-mix(in_srgb,var(--theme-danger)_30%,transparent)] hover:bg-[color-mix(in_srgb,var(--theme-danger)_10%,transparent)] text-[var(--theme-danger)] transition-colors"
          >
            Delete
          </button>
          <button
            onClick={clearSelection}
            className="px-2.5 py-1 rounded-lg border border-[var(--theme-border)] hover:bg-[var(--theme-hover)] text-[var(--theme-muted)] transition-colors"
          >
            ✕
          </button>
        </div>
      ...
      {panelLive && (
        <TaskDetailPanel
          ...
          onEdit={() => {
            setPanelTask(null)
            setEditingTask(panelLive)
          }}
          onExecute={async () => {
            const live = panelLive
            setExecutingTaskId(live.id)
            try {
              await executeTask(live.id)
              await tasksQuery.refetch()
              toast(`Agent is working on "${live.title}"…`)
            } catch {
              toast('Failed to start agent', { type: 'error' })
            } finally {
              setExecutingTaskId(null)
            }
          }}
      ...
      {showActivity && (
        <ActivityPanel
          ...
          onTabChange={(tab) => {
            setActivityTab(tab)
            if (tab === 'feed') markNotifSeen()
          }}
          inboxTasks={stats.inboxTasks}
          inboxReplies={inboxReplies}
          onReplyChange={(taskId, value) =>
            setInboxReplies((prev) => ({ ...prev, [taskId]: value }))
          }
          onReplySubmit={async (taskId) => {
            ...
            try {
              ...
              toast('Reply failed', { type: 'error' })
            } finally {
              setInboxSending(null)
            }
          }}
        ...
        <TagsBrowserPanel
          ...
          onBulkTagSubmit={async () => {
            ...
            try {
              ...
              toast('Tagging failed', { type: 'error' })
            } finally {
              setTaggingSelected(false)
            }
          }}
        />
      )}
        ...
        <SisterRebalanceModal
          ...
          onConfirm={async () => {
            if (
              !rebalanceFrom ||
              !rebalanceTo ||
              rebalanceFrom === rebalanceTo
            ) {
              toast('Pick two different sisters', { type: 'error' })
              return
            }
            setRebalancing(true)
            try {
              const candidates = tasks
                .filter(
                  (t) =>
                    t.assignee === rebalanceFrom &&
                    (t.column === 'todo' || t.column === 'backlog') &&
                    !t.agent_state,
                )
              ...
              for (const t of candidates) {
                await quickUpdateMutation.mutateAsync({
                  id: t.id,
                  input: { assignee: rebalanceTo },
                })
              }
              invalidate()
              toast(
                `Moved ${candidates.length} tasks from ${rebalanceFrom} → ${rebalanceTo}`,
              )
              setShowRebalance(false)
            } catch {
              toast('Rebalance failed', { type: 'error' })
            } finally {
      ...
      {showTimeoutAnalysis && timeoutAnalysisData && (
        <TimeoutAnalysisModal
          ...
          onFilterTimedOut={() => {
            setShowTimeoutAnalysis(false)
            setFilterTimedOut(true)
          }}
          onRescueAll={async () => {
            setRescuingTimedOut(true)
            try {
              const res = await fetch('/api/tasks-rescue-timedout', {
                method: 'POST',
              })
              const data = (await res.json()) as {
                ok: boolean
          ...
          onConfirm={async () => {
            ...
            try {
              const res = await fetch('/api/tasks-stale', {
                method: 'POST',
                headers: { 'Content-Type': 'application/json' },
                body: JSON.stringify({ age_days: archiveDays }),
              })
              const data = (await res.json()) as {
                ok: boolean
                archived: number
              ...
              if (data.ok) {
                await tasksQuery.refetch()
                toast(
                  `Archived ${data.archived} stale task${data.archived !== 1 ? 's' : ''}`,
                )
          ...
          onConfirm={async () => {
            ...
            try {
              ...
              if (data.ok) {
                await tasksQuery.refetch()
                toast(
                  `Unlocked ${data.unblocked ?? count} task${(data.unblocked ?? count) !== 1 ? 's' : ''} — deploy sweep triggered`,
                )
          ...
          onConfirm={() => {
            ...
            void fetch('/api/tasks-prune', { method: 'POST' })
              .then(async (res) => {
                ...
                invalidate()
                toast(
                  `Pruned ${data.pruned} stale task${data.pruned !== 1 ? 's' : ''}`,
                )
              })
              .catch(() => toast('Prune failed', { type: 'error' }))
              .finally(() => setPruningStale(false))
          }}
        />
      )}
        ...
        <ConfirmDialog
          ...
          onConfirm={() => {
            ...
            void Promise.all(ids.map((id) => deleteTask(id)))
              ...
              .catch(() =>
                toast('Failed to delete selected tasks', { type: 'error' }),
              )
          }}
        />
      )}
