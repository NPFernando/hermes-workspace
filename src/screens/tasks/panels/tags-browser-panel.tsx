export function TagsBrowserPanel({
  ...
  return (
    <>
      ...
      <div
        className="fixed right-0 top-0 bottom-0 z-50 w-[min(320px,92vw)] flex flex-col border-l border-[var(--theme-border)] shadow-2xl"
      ...
      >
        ...
        {selectedCount > 0 && (
          <div className="px-4 py-3 border-b border-[var(--theme-border)] bg-[var(--theme-hover)]">
            <p className="text-[10px] text-[var(--theme-muted)] mb-1.5">
              {selectedCount} tasks selected — add tag:
            </p>
                ...
                <div key={tag} className="flex items-center gap-2">
                  ...
                  >
                    <div className="flex-1 h-1.5 rounded-full bg-[var(--theme-hover)] overflow-hidden">
                      <div
                        className="h-full rounded-full bg-[var(--theme-accent)] opacity-60"
                        style={{ width: `${(count / maxCount) * 100}%` }}
                      />
                    </div>
                    <span className="text-[11px] text-[var(--theme-muted)] shrink-0 w-28 truncate">
                      #{tag}
                    </span>
                    <span className="text-[11px] font-medium text-[var(--theme-text)] shrink-0 w-8 text-right">
                      {count}
                    </span>
                  </button>
                </div>
