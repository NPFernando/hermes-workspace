...
export function ActivityPanel({
  ...
  return (
    <>
      ...
      <div
        className="fixed right-0 top-0 bottom-0 z-50 w-[min(440px,94vw)] flex flex-col border-l border-[var(--theme-border)] shadow-2xl"
      ...
      >
        ...
        <div className="flex border-b border-[var(--theme-border)]">
          ...
          >
            📥 Action Required
            {inboxTasks.length > 0 ? ` (${inboxTasks.length})` : ''}
          </button>
          <button
            type="button"
          ...
          >
            ...
            {unreadNotifCount > 0 && activityTab !== 'feed' && (
              <span className="ml-1 text-[9px] px-1 py-0.5 rounded-full bg-[color-mix(in_srgb,var(--theme-accent-secondary)_20%,transparent)] text-[var(--theme-accent-secondary)] border border-[color-mix(in_srgb,var(--theme-accent-secondary)_30%,transparent)]">
                {unreadNotifCount}
              </span>
            )}
          </button>
        ...
        {activityTab === 'inbox' && (
          <div className="flex-1 min-h-0 overflow-y-auto scrollbar-thin divide-y divide-[var(--theme-border)]">
            {inboxTasks.length === 0 && (
              <div className="text-center py-12 text-xs text-[var(--theme-muted)] opacity-50">
                No tasks waiting for your input
              </div>
            )}
            {inboxTasks.map((t) => {
              const lastAgentMsg = [...(t.agent_history ?? [])]
                .reverse()
                .find((h) => h.action !== 'rescued' && h.note)
              const ago = t.agent_action_at
                ? relativeTime(t.agent_action_at)
                : ''
              return (
                <div key={t.id} className="p-4">
                  <div className="flex items-start gap-2 mb-2">
                    <div className="flex-1 min-w-0">
                      ...
                      <div className="flex items-center gap-2 mt-0.5">
                        <span className="text-[9px] text-[var(--theme-muted)] opacity-50 capitalize">
                          {t.assignee ?? 'unassigned'}
                        </span>
                        {ago && (
                          <span className="text-[9px] text-[var(--theme-muted)] opacity-40">
                            {ago}
                          </span>
                        )}
                      </div>
                  ...
                  <div className="flex gap-2">
                    <input
                      ...
                      onKeyDown={(e) => {
                        if (e.key === 'Enter') onReplySubmit(t.id)
                      }}
                    ...
                    />
                    {inboxSending === t.id && (
                      <span className="text-[10px] text-[var(--theme-muted)] self-center">
                        Sending…
                      </span>
                    )}
                  </div>
        ...
        {activityTab === 'feed' && (
          <div className="flex-1 overflow-y-auto scrollbar-thin p-3 flex flex-col gap-2">
            {notifEvents.length === 0 && (
              <div className="text-center py-8 text-xs text-[var(--theme-muted)] opacity-50">
                No activity in the last 24h
              </div>
            )}
            {notifEvents.map((ev, i) => {
              const isNew = ev.at > notifLastSeen
              const ago = relativeTime(ev.at)
              const icon =
                ev.action === 'completed'
                  ? '✅'
                  : ev.action === 'blocked'
              ...
              const color =
                ev.action === 'completed'
                  ...
                  : ev.action === 'blocked' || ev.action === 'timed_out'
              ...
              return (
                <button
                  ...
                  className={cn(
                    'text-left px-3 py-2 rounded-lg border transition-colors hover:bg-[var(--theme-hover)]',
                    isNew
                      ? 'border-[color-mix(in_srgb,var(--theme-accent-secondary)_30%,transparent)] bg-[color-mix(in_srgb,var(--theme-accent-secondary)_5%,transparent)]'
                ...
                >
                  <div className="flex items-center gap-2">
                    ...
                    <span className="text-[9px] text-[var(--theme-muted)] opacity-50 ml-auto">
                      {ago}
                    </span>
                  </div>
                  <p className="text-[11px] text-[var(--theme-text)] truncate mt-0.5">
                    {ev.taskTitle.slice(0, 60)}
                  </p>
                  {ev.note && (
                    <p className="text-[9px] text-[var(--theme-muted)] opacity-60 truncate mt-0.5">
                      {ev.note.slice(0, 80)}
                    </p>
                  )}
                </button>
