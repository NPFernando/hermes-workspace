...
export function TimeoutAnalysisModal({
  ...
  return (
    <>
      ...
      <div
        className="fixed left-1/2 top-1/2 -translate-x-1/2 -translate-y-1/2 z-50 w-[min(480px,94vw)] max-h-[80vh] rounded-xl border border-[var(--theme-border)] shadow-2xl flex flex-col"
      ...
      >
        <div className="flex items-center justify-between px-5 py-4 border-b border-[var(--theme-border)]">
          <h2 className="text-sm font-semibold text-[var(--theme-text)]">
            ⏱ Timeout Analysis — today ({data.timedOutEntries.length} total)
          </h2>
          <button
            onClick={onClose}
        ...
        <div className="overflow-y-auto scrollbar-thin p-5 flex flex-col gap-5">
              ...
              {data.topAssignees.map(([name, count]) => (
                <div key={name} className="flex items-center gap-2 mb-1.5">
                  <div className="flex-1 h-1.5 rounded-full bg-[var(--theme-hover)] overflow-hidden">
                    <div
                      className="h-full rounded-full bg-[var(--theme-warning)]"
                      style={{
                        width: `${(count / data.timedOutEntries.length) * 100}%`,
                      }}
                    />
                  </div>
                  <span className="text-[11px] text-[var(--theme-muted)] capitalize shrink-0 w-20 truncate">
                    {name}
                  </span>
                  <span className="text-[11px] font-medium text-[var(--theme-warning)] shrink-0">
                    {count}
                  </span>
                </div>
              ))}
            ...
            <div>
              ...
              {data.topTags.map(([tag, count]) => (
                <div key={tag} className="flex items-center gap-2 mb-1.5">
                  ...
                  <span className="text-[11px] text-[var(--theme-muted)] shrink-0 w-20 truncate">
                    {tag}
                  </span>
                  <span className="text-[11px] font-medium text-[var(--theme-warning)] shrink-0">
                    {count}
          ...
          <div>
            ...
            {data.sample.map((e, i) => (
              <div
                key={i}
                className="mb-2 pb-2 border-b border-[var(--theme-border)]/50 last:border-0"
              >
                <button
                  type="button"
                ...
                <p className="text-[9px] text-[var(--theme-muted)] opacity-40 mt-0.5 capitalize">
                  {e.task.assignee ?? 'unassigned'} ·{' '}
                  {e.task.tags.slice(0, 2).join(', ') || 'no tags'}
                </p>
              </div>
            ))}
          </div>
