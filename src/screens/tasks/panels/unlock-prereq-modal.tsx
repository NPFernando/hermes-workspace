      ...
      <div className="relative z-10 w-full max-w-md bg-[var(--theme-card)] border border-[var(--theme-border)] rounded-xl shadow-2xl">
        ...
        <div className="px-5 py-4 flex flex-col gap-3">
          <p className="text-xs text-[var(--theme-muted)]">
            Marking this prerequisite as{' '}
            <strong className="text-[var(--theme-warning)]">done</strong> will immediately
            unlock{' '}
            <strong className="text-[var(--theme-text)]">
              {prereq.count} waiting task{prereq.count !== 1 ? 's' : ''}
            </strong>{' '}
            and trigger a deploy sweep.
          </p>
          <div className="px-3 py-2 rounded-lg bg-[color-mix(in_srgb,var(--theme-warning)_10%,transparent)] border border-[color-mix(in_srgb,var(--theme-warning)_20%,transparent)]">
            <p className="text-xs font-medium text-[var(--theme-warning)]">
              {prereq.title.slice(0, 100)}
            </p>
            <p className="text-[9px] text-[color-mix(in_srgb,var(--theme-warning)_60%,transparent)] mt-0.5">
              {prereq.count} task{prereq.count !== 1 ? 's' : ''} waiting on this
            </p>
          </div>
          <div className="flex gap-3 mt-2">
            ...
            >
              {unlocking
                ? 'Unlocking…'
                : `Confirm — unlock ${prereq.count} tasks`}
            </button>
          </div>
        </div>
      </div>
