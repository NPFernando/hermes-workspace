export function SisterRebalanceModal({
  ...
  return (
    <>
      ...
      <div
        className="fixed left-1/2 top-1/2 -translate-x-1/2 -translate-y-1/2 z-50 w-[min(380px,92vw)] rounded-xl border border-[var(--theme-border)] shadow-2xl p-5"
        style={{ background: 'var(--theme-panel)' }}
      >
        <div className="flex items-center justify-between mb-4">
          <h2 className="text-sm font-semibold text-[var(--theme-text)]">
            ⇄ Rebalance Sister Load
          </h2>
          <button
            onClick={onClose}
        ...
        <div className="flex flex-col gap-3">
          <div className="flex gap-2 items-center">
            <div className="flex-1">
              <label className="text-[10px] text-[var(--theme-muted)] block mb-1">
                From (overloaded)
              </label>
              <select
                value={rebalanceFrom}
          ...
          <div>
            <label className="text-[10px] text-[var(--theme-muted)] block mb-1">
              Move up to N tasks (todo/backlog only)
            </label>
            <input
              type="number"
              min={1}
