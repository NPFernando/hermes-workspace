      ...
      <div className="relative z-10 w-full max-w-md bg-[var(--theme-card)] border border-[var(--theme-border)] rounded-xl shadow-2xl flex flex-col max-h-[80vh]">
        ...
        <div className="px-5 py-4 flex flex-col gap-4">
          <p className="text-xs text-[var(--theme-muted)]">
            Move inactive todo/backlog tasks to done. Only tasks with zero agent
            activity since the cutoff are affected.
          </p>
          <div className="grid grid-cols-3 gap-3">
            {([30, 60, 90] as const).map((bucketDays) => {
              const count =
                bucketDays === 30
                  ? preview.buckets.days30
                  : bucketDays === 60
                    ? preview.buckets.days60
                    : preview.buckets.days90
              return (
                <button
                  key={bucketDays}
                  type="button"
          ...
          <div>
            <p className="text-[10px] text-[var(--theme-muted)] mb-2">
              Oldest inactive tasks (preview):
            </p>
            <div className="flex flex-col gap-1 max-h-40 overflow-y-auto scrollbar-thin">
              {preview.previews
                .filter((p) => p.ageDays >= days)
                .slice(0, 10)
                .map((p) => (
                  <div
                    key={p.id}
                    className="flex items-center gap-2 px-2 py-1 rounded bg-[var(--theme-hover)]"
                  >
              ...
              {preview.previews.filter((p) => p.ageDays >= days).length ===
                0 && (
                <p className="text-xs text-[var(--theme-muted)] opacity-50 text-center py-2">
                  No tasks older than {days} days
                </p>
              )}
            </div>
          </div>
        ...
        <div className="flex gap-3 px-5 py-4 border-t border-[var(--theme-border)]">
          ...
          <button
            ...
            onClick={onConfirm}
            className="flex-1 text-xs rounded-lg border border-[color-mix(in_srgb,var(--theme-danger)_40%,transparent)] bg-[color-mix(in_srgb,var(--theme-danger)_10%,transparent)] px-3 py-2 text-[var(--theme-danger)] hover:bg-[color-mix(in_srgb,var(--theme-danger)_20%,transparent)] transition-colors disabled:opacity-40"
          >
            {archiving ? 'Archiving…' : `Archive ${bucketCount} tasks`}
          </button>
        </div>
      </div>
    </div>
