...
const SWARM2_OPERATION_THEME: CSSProperties = {
  ...
  ['--theme-success' as string]: 'var(--color-green-600, #16a34a)',
    ...
    >
      ...
      <div className="relative z-10 flex flex-col items-center gap-4">
        ...
        <div className="relative w-full pt-3">
          ...
          >
            {!tmuxAvailable ? (
              <div className="rounded-xl border border-[color-mix(in_srgb,var(--theme-warning)_40%,transparent)] bg-[color-mix(in_srgb,var(--theme-warning)_10%,transparent)] px-4 py-2.5 text-xs text-[var(--theme-warning)]">
                <div className="font-semibold text-[var(--theme-warning)]">
                  tmux not installed on this host
                </div>
                <div className="mt-1 text-[color-mix(in_srgb,var(--theme-warning)_80%,transparent)]">
                ...
                <code className="mt-1 inline-block rounded bg-black/30 px-2 py-0.5 text-[10px] text-[var(--theme-warning)]">
                  brew install tmux
                </code>{' '}
                <span className="text-[color-mix(in_srgb,var(--theme-warning)_60%,transparent)]">(macOS) or</span>{' '}
                <code className="inline-block rounded bg-black/30 px-2 py-0.5 text-[10px] text-[var(--theme-warning)]">
                  apt install tmux
                </code>{' '}
                <span className="text-[color-mix(in_srgb,var(--theme-warning)_60%,transparent)]">(Ubuntu/Debian).</span>
              </div>
            ) : null}
            {focusedRuntimeWorkerId ? (
                          ...
                          {runtime?.tmuxAttachable ? (
                            <>
                              ...
                              <button
                                ...
                                className={cn(
                                  'rounded-full border border-[var(--theme-danger)] bg-[color-mix(in_srgb,var(--theme-danger)_10%,transparent)] px-1.5 py-0.5 text-[10px] uppercase tracking-[0.14em] text-[var(--theme-danger)] transition-colors hover:opacity-90',
                                  pendingTmux.has(member.id) &&
                                    'cursor-not-allowed opacity-50',
                                )}
                                title={`Stop the live agent session in tmux (swarm-${member.id})`}
                              >
                                {pendingTmux.has(member.id)
                                  ? 'Stopping…'
                                  : 'Stop'}
  ...
  const startAgentSession = useCallback(
    async (workerId: string) => {
      ...
      try {
        ...
        toast(`Failed to start ${workerId}: ${safeErrorMessage(err)}`, {
          type: 'error',
        })
      } finally {
        setPendingTmux((prev) => {
            ...
            <div className="relative flex shrink-0 items-center gap-2 text-sm text-[var(--theme-muted)]">
              ...
              >
                ...
                {actionableNotificationCount > 0 ? (
                  <span className="absolute -right-1 -top-1 inline-flex min-w-5 items-center justify-center rounded-full bg-[var(--theme-danger)] px-1.5 py-0.5 text-[10px] font-bold text-white">
      ...
      {addSwarmOpen ? (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/35 px-4 py-6 backdrop-blur-sm">
          <div className="w-full max-w-2xl rounded-3xl border border-[var(--theme-border2)] bg-[var(--theme-card)] p-6 shadow-[0_30px_100px_var(--theme-shadow)]">
            ...
            {addSwarmError ? (
              <div className="mt-3 rounded-xl border border-[color-mix(in_srgb,var(--theme-danger)_30%,transparent)] bg-[color-mix(in_srgb,var(--theme-danger)_10%,transparent)] px-3 py-2 text-sm text-[var(--theme-danger)]">
                {addSwarmError}
              </div>
            ) : null}
