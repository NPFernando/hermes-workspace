...
function PersonalityBadge({
  ...
  const sister =
    sistersByRole[role.toLowerCase()] ??
    (workerId ? sistersByRole[workerId.toLowerCase()] : undefined)
...
export function roleFromId(id: string): string {
  const m = id.match(/(\d+)/)
  const n = m ? m[1] : ''
  switch (n) {
    case '1':
...
export function OperationalWorkerCard({
  ...
  return (
    ...
    >
      {!settingsOpen ? (
        <>
          {/* Below md the three header groups wrap into two rows (name on top, pills + role
          below) instead of the absolute-positioned single-row layout, which physically
          overlaps the centered name on narrow cards (left+right groups can claim 20rem). */}
          <div className="relative flex min-h-8 flex-wrap items-center gap-y-1 md:flex-nowrap">
            ...
            <div className="order-1 flex w-full justify-center px-2 md:order-none md:px-28">
              ...
              {recentSummary ? (
                <p
                  className="mt-0.5 max-w-full truncate text-center text-[10px] text-[var(--theme-muted)]"
                  title={
                    outputFreshness
                      ? `${recentSummary} · ${outputFreshness}`
                      : recentSummary
                  }
                >
                  {recentSummary}
            ...
            <div className="order-3 ml-auto flex min-w-0 max-w-[10rem] flex-col items-end gap-0.5 md:absolute md:right-0 md:order-none md:ml-0">
      ...
      {settingsOpen ? (
        <div
          className="absolute inset-0 z-20 flex items-center justify-center rounded-[1.35rem] bg-[color-mix(in_srgb,var(--theme-bg)_90%,transparent)] backdrop-blur-sm p-3"
          onClick={(event) => {
            event.stopPropagation()
