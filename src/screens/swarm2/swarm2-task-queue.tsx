...
function statusTone(column?: string | null): {
  ...
  if (c === 'blocked') return { dot: 'bg-[var(--theme-danger)]', rank: 0 }
  if (c === 'in_progress' || c === 'doing' || c === 'progress')
    return { dot: 'bg-[var(--theme-success)]', rank: 1 }
  if (c === 'review' || c === 'reviewing')
    return { dot: 'bg-[var(--theme-accent-secondary)]', rank: 2 }
  return { dot: 'bg-[var(--theme-warning)]', rank: 3 }
}

function priorityTone(priority?: string | null) {
  const p = (priority ?? '').toLowerCase()
  if (p === 'high') {
    return 'border-[color-mix(in_srgb,var(--theme-danger)_40%,transparent)] bg-[color-mix(in_srgb,var(--theme-danger)_10%,transparent)] text-[var(--theme-danger)]'
  }
  if (p === 'medium') {
    return 'border-[color-mix(in_srgb,var(--theme-warning)_40%,transparent)] bg-[color-mix(in_srgb,var(--theme-warning)_10%,transparent)] text-[var(--theme-warning)]'
  }
  if (p === 'low') {
    return 'border-[color-mix(in_srgb,var(--theme-success)_40%,transparent)] bg-[color-mix(in_srgb,var(--theme-success)_10%,transparent)] text-[var(--theme-success)]'
  }
  return null
}
