...
import type {
  ...
  AgentWorkingStatus,
  ModelPresetId,
} from '@/screens/gateway/shared-types'
...
export const OFFICE_MODEL_BADGE: Record<ModelPresetId, string> = {
  auto: 'rounded-full border border-[var(--theme-border)] bg-[var(--theme-card)] text-[var(--theme-muted)]',
  ...
  switch (status) {
    ...
    case 'spawning':
      return {
        ...
        dotClassName: 'bg-[var(--theme-accent-secondary)]',
        pulse: true,
      }
    case 'paused':
      return {
...
function RemoteSessionCard({
  ...
  return (
    <button
      type="button"
      onClick={onClick}
      className="group flex min-h-11 flex-col items-center gap-1.5 rounded-xl border border-[var(--theme-border)] bg-[var(--theme-card)] p-3 text-center transition-all hover:border-[var(--theme-accent)] hover:shadow-sm"
    >
      <div className="relative">
        <div className="h-8 w-8 rounded-full bg-[var(--theme-card2)] flex items-center justify-center text-lg">
          🤖
        </div>
        <span
          className={cn(
      ...
      <span className="w-full truncate text-sm font-semibold text-[var(--theme-text)]">
        {session.label}
      </span>
      {modelDisplay ? (
        <span className="w-full truncate text-xs text-[var(--theme-muted)]">
          {modelDisplay}
        </span>
      ) : null}
      {lastMessageSnippet ? (
        <span className="w-full truncate text-xs italic text-[var(--theme-muted)]">
          {lastMessageSnippet}
        </span>
      ) : null}
      <div className="flex items-center gap-1.5 flex-wrap justify-center">
        ...
        {session.startedAt ? (
          <span className="text-xs text-[var(--theme-muted)] tabular-nums">
...
export function OfficeView({
  ...
  return (
    ...
    >
      {/* Header bar */}
      {hideHeader ? null : (
        <div className="flex shrink-0 flex-wrap items-start justify-between gap-2 border-b border-[var(--theme-border)] bg-[var(--theme-panel)]/80 px-5 py-3 backdrop-blur">
          <div className="flex min-w-0 flex-1 flex-col gap-1">
            <span className="text-base font-bold text-[var(--theme-text)] dark:text-white">
              ClawSuite Office
            </span>
            <div className="flex flex-wrap items-center gap-2">
              <span className="rounded-full bg-[var(--theme-card)] px-2 py-0.5 text-[10px] font-medium text-[var(--theme-muted)] tabular-nums">
                {agentRows.length} agents
              </span>
              <span className="rounded-full bg-[color-mix(in_srgb,var(--theme-success)_10%,transparent)] px-2 py-0.5 text-[10px] font-medium text-[var(--theme-success)] tabular-nums">
                {activeCount} working
              </span>
              <span className="rounded-full bg-[color-mix(in_srgb,var(--theme-accent-secondary)_10%,transparent)] px-2 py-0.5 text-[10px] font-medium text-[var(--theme-accent-secondary)] tabular-nums">
          ...
          <div className="flex items-center gap-2">
            {missionRunning ? (
              <span className="flex items-center gap-1.5 rounded-full border border-[color-mix(in_srgb,var(--theme-success)_25%,transparent)] bg-[color-mix(in_srgb,var(--theme-success)_10%,transparent)] px-2.5 py-1 text-[10px] font-semibold text-[var(--theme-success)]">
                <span className="relative flex size-1.5">
                  <span className="absolute inset-0 animate-ping rounded-full bg-[color-mix(in_srgb,var(--theme-success)_60%,transparent)]" />
                  <span className="relative inline-flex size-1.5 rounded-full bg-[var(--theme-success)]" />
                </span>
                Mission Live
              </span>
            ) : null}
          ...
          {agentRows.map((agent, index) => {
            ...
            return (
              <button
                ...
                className="flex min-h-11 w-full items-center gap-3 rounded-xl border border-[var(--theme-border)] bg-[var(--theme-card)] px-3 py-2 text-left shadow-sm"
              >
                <div
                  className={cn(
                ...
                <div className="min-w-0 flex-1">
                  <p className="truncate text-sm font-semibold text-[var(--theme-text)] dark:text-white">
                    {agent.name}
                  </p>
                  <p className="truncate text-xs text-[var(--theme-muted)] dark:text-slate-400">
      ...
      <div className="hidden shrink-0 justify-end px-3 pb-1 pt-2 md:flex">
        <div className="relative" data-layout-picker>
          <button
            ...
            className="inline-flex min-h-11 items-center rounded-lg border border-[var(--theme-border)] bg-[var(--theme-card)] px-3 py-1.5 text-xs font-semibold text-[var(--theme-text)] transition-colors hover:bg-[var(--theme-card2)] sm:px-4 sm:py-2 sm:text-sm"
            title="Change office layout"
          >
            <span>✏️</span>
          </button>
          {layoutPickerOpen && (
            <div className="absolute right-0 top-full z-50 mt-1 min-w-[120px] overflow-hidden rounded-xl glass-dropdown shadow-lg">
              {LAYOUT_TEMPLATE_OPTIONS.map((opt) => (
                <button
                  ...
                  className={cn(
                    'w-full px-3 py-2 text-left text-[12px] transition-colors hover:bg-[var(--theme-card)] dark:hover:bg-slate-800',
                    layoutTemplate === opt.key
                      ? 'font-medium text-accent-600'
                      : 'text-[var(--theme-text)] dark:text-slate-300',
                  )}
                >
                  {opt.label}
                </button>
      ...
      >
        ...
        {!compact && !hideHeader ? (
          <div className="pointer-events-none absolute left-1/2 top-3 z-20 -translate-x-1/2">
            <div className="rounded-md border border-[var(--theme-border)]/90 bg-[#fdfdf8] px-4 py-2 shadow-[0_2px_8px_rgba(15,23,42,0.15)]">
              <span className="block whitespace-nowrap text-center text-sm font-bold tracking-wide text-[var(--theme-text)] [font-family:'Bradley_Hand','Marker_Felt','Comic_Sans_MS',cursive]">
                {companyName}
              </span>
            </div>
          </div>
        ...
        {agentRows.map((agent, index) => {
          ...
          return (
            ...
            >
              {/* Speech bubble */}
              {showSpeech ? (
                <span className="pointer-events-none relative mb-2 max-w-[180px] rounded-lg bg-[var(--theme-panel)] px-3 py-1.5 text-xs leading-snug text-[var(--theme-text)] shadow-lg">
                  <span className="block truncate">{speechLine}</span>
                  <span className="absolute left-1/2 top-full h-2.5 w-2.5 -translate-x-1/2 -translate-y-1/2 rotate-45 bg-[var(--theme-panel)]" />
                </span>
              ) : null}
              ...
              {/* Activity indicator */}
              {isActive ? (
                <span className="mt-1 flex items-center gap-0.5 rounded-full bg-[color-mix(in_srgb,var(--theme-success)_10%,transparent)] px-1.5 py-0.5 text-[10px] font-semibold text-[var(--theme-success)]">
                  <span className="size-1 animate-pulse rounded-full bg-[var(--theme-success)]" />
                  <span className="size-1 animate-pulse rounded-full bg-[var(--theme-success)] [animation-delay:120ms]" />
                  <span className="size-1 animate-pulse rounded-full bg-[var(--theme-success)] [animation-delay:240ms]" />
                  <span className="ml-0.5">Working</span>
                </span>
              ) : isIdle && !pos.atDesk && !compact ? (
                <span className="mt-1 rounded-full bg-[color-mix(in_srgb,var(--theme-accent-secondary)_10%,transparent)] px-1.5 py-0.5 text-[10px] font-medium text-[var(--theme-accent-secondary)]">
                  On break
                </span>
              ) : null}

              {/* Name + model */}
              <span className="mt-1 max-w-full truncate text-[10px] font-semibold text-[var(--theme-text)] dark:text-white">
                {agent.name}
              </span>
              {!compact ? (
                <span className="max-w-full truncate text-xs text-[var(--theme-muted)] dark:text-slate-400">
                  {getOfficeModelLabel(agent.modelId)}
                </span>
              ) : null}
            </button>
      ...
      {!compact ? (
        <div className="hidden items-center justify-between border-t border-[var(--theme-border)] bg-white/80 px-4 py-2 text-xs text-[var(--theme-muted)] backdrop-blur dark:border-slate-700 dark:bg-slate-800/80 dark:text-slate-400 md:flex">
          <span>
          ...
          <div className="flex items-center gap-4">
            ...
            <span className="flex items-center gap-1">
              <span className="size-2 rounded-full bg-[var(--theme-muted)]" />{' '}
              Idle
            </span>
            <span className="flex items-center gap-1">
              <span className="size-2 rounded-full bg-[var(--theme-danger)]" />{' '}
              Error
            </span>
            <span className="flex items-center gap-1">
              <span className="size-2 rounded-full bg-[var(--theme-muted)]" />{' '}
