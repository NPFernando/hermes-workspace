...
export function getWorkerDot(
  status: 'running' | 'complete' | 'stale' | 'idle',
) {
  if (status === 'complete')
    return { dotClass: 'bg-[var(--theme-success)]', label: 'Complete' }
  if (status === 'running')
    return {
      dotClass: 'bg-[var(--theme-accent-secondary)] animate-pulse',
      label: 'Running',
    }
  if (status === 'idle')
    return { dotClass: 'bg-[var(--theme-warning)]', label: 'Idle' }
  return { dotClass: 'bg-[var(--theme-danger)]', label: 'Stale' }
}

export function getWorkerBorderClass(
  status: 'running' | 'complete' | 'stale' | 'idle',
) {
  if (status === 'complete') return 'border-l-[var(--theme-success)]'
  if (status === 'running') return 'border-l-[var(--theme-accent-secondary)]'
  if (status === 'idle') return 'border-l-[var(--theme-warning)]'
  return 'border-l-[var(--theme-danger)]'
}
