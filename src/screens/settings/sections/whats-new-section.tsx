      ...
      <div className="space-y-5">
        {CHANGELOG.map((entry, idx) => (
          <div
            key={entry.version}
            className={
              idx > 0 ? 'border-t border-[var(--theme-border)] pt-5' : ''
            }
          >
            {/* Version header */}
            <div className="mb-3 flex flex-wrap items-center gap-2">
              ...
              <span className="flex-1 text-xs text-[var(--theme-text)] font-medium">
                {entry.summary}
              </span>
            </div>
            ...
            <div className="space-y-2">
              {entry.changes.map((c, i) => (
                <div key={i} className="flex items-start gap-2.5">
                  <span
                    className={`mt-0.5 shrink-0 rounded-full px-1.5 py-0.5 text-[10px] font-semibold leading-none ${KIND_CLASS[c.kind]}`}
                  >
                    {KIND_LABEL[c.kind]}
                  </span>
                  <span className="text-xs text-[var(--theme-muted)] leading-relaxed">
                    {c.text}
                  </span>
                </div>
              ))}
