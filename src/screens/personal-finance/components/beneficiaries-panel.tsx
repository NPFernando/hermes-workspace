...
export function BeneficiariesPanel({
  ...
  const [editDrafts, setEditDrafts] = useState<
    Record<string, { name: string; relationship: string; note: string }>
  >({})
  ...
  async function deleteBeneficiary(id: string) {
    const data = await post(
      { action: 'delete_record', kind: 'beneficiary', id },
      `delete-${id}`,
    )
  ...
  return (
    <section className="mt-6 rounded-3xl border border-[var(--theme-border)] bg-[var(--theme-panel)]/70 p-5">
      ...
      <p className="text-xs text-[var(--theme-muted)]">
        Informal notes on who you'd want to receive what — not legally binding,
        purely a personal reference.
      </p>
                ...
                <div className="flex flex-wrap items-center gap-2">
                  ...
                  <input
                    ...
                    onChange={(e) =>
                      setEditDrafts((prev) => ({
                        ...prev,
                        [id]: { ...prev[id], relationship: e.target.value },
                      }))
                    }
                    className={inputClass}
                  />
