  ...
  if (sr.targetPct === 0) {
    return (
      <section className="mt-6 rounded-3xl border border-[var(--theme-border)] bg-[var(--theme-panel)]/70 p-5">
        <h2 className="text-lg font-semibold text-[var(--theme-text)]">Savings rate target</h2>
        <p className="mt-1 text-xs text-[var(--theme-muted)]">
          No target set yet. Choose what share of income you want to save each month.
        </p>
        <div className="mt-3 flex items-center gap-2">
          <input
            ...
            className="w-20 rounded-lg border border-[var(--theme-border)] bg-black/20 px-2 py-1 text-sm text-[var(--theme-text)]"
          />
    ...
    <section className="mt-6 rounded-3xl border border-[var(--theme-border)] bg-[var(--theme-panel)]/70 p-5">
      ...
      {!sr.hasData && (
        <p className="mt-1 text-xs text-amber-300/80">
          No complete month of income/expense history yet — actual rate will fill in once one is available.
        </p>
      )}
    </section>
  )
