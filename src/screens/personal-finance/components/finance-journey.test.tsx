...
describe('personal finance dashboard journey', () => {
  ...
  it('shows forecasts and freshness, then holds a possible CSV duplicate for explicit review', async () => {
    ...
    const rateMove = container.querySelector(
      'select[aria-label="Foreign currency rate move against LKR"]',
    ...
    await act(async () => {
      rateMove.value = '-10'
      rateMove.dispatchEvent(new Event('change', { bubbles: true }))
    })
    expect(rateMove.value).toBe('-10')
    expect(fxOnlyChange()).not.toBe(initialFxScenario)
    expect(payload.fxGainLoss.totalReturnLkr).toBe(1_500)

    const breakdown = [...container.querySelectorAll('button')].find(
      (button) => button.textContent === 'breakdown',
    )
    expect(breakdown).toBeDefined()
    await act(async () =>
      breakdown?.dispatchEvent(new MouseEvent('click', { bubbles: true })),
    )
    ...
    const importButton = [...container.querySelectorAll('button')].find(
      (button) => button.textContent.startsWith('Import 1 row'),
    )
    ...
    const reviewedImport = [...container.querySelectorAll('button')].find(
      (button) => button.textContent.includes('reviewed row(s) anyway'),
    )
    expect(reviewedImport).toBeDefined()
    await act(async () =>
      reviewedImport?.dispatchEvent(new MouseEvent('click', { bubbles: true })),
    )
    await act(async () => {
      await Promise.resolve()
      await Promise.resolve()
