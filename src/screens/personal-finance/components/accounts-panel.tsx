...
export function AccountsPanel({
  ...
  async function deleteAccount(id: string) {
    const data = await post(
      { action: 'delete_record', kind: 'account', id },
      `delete-${id}`,
    )
  ...
  for (const account of accounts) {
    ...
    totalsByCurrency.set(
      accountCurrency,
      (totalsByCurrency.get(accountCurrency) ?? 0) +
        numberField(account, 'balance'),
    )
  }
  ...
  return (
    <section className="mt-6 rounded-3xl border border-[var(--theme-border)] bg-[var(--theme-panel)]/70 p-5">
      ...
      <p className="text-xs text-[var(--theme-muted)]">
        Bank, cash, cards, wallets, brokers, and loans — everything else in this
        app ties back to these.
      </p>
      {totalsText && (
      ...
      <div className="mt-4 grid gap-2">
        ...
        {accounts.map((account, index) => {
          ...
          const openingBalanceValue = optionalNumberField(
            account,
            'openingBalance',
          )
          const openingBalanceDateValue = stringField(
            account,
            'openingBalanceDate',
          )
                ...
                <div className="flex flex-wrap items-center gap-2">
                  ...
                  <input
                    ...
                    onChange={(e) =>
                      setEditDrafts((prev) => ({
                        ...prev,
                        [id]: { ...prev[id], balance: e.target.value },
                      }))
                    }
                    className={`${inputClass} w-32`}
                  />
                  <input
                    ...
                    onChange={(e) =>
                      setEditDrafts((prev) => ({
                        ...prev,
                        [id]: { ...prev[id], openingBalance: e.target.value },
                      }))
                    }
                  ...
                  <input
                    ...
                    onChange={(e) =>
                      setEditDrafts((prev) => ({
                        ...prev,
                        [id]: { ...prev[id], maskedIdentifier: e.target.value },
                      }))
                    }
                    className={inputClass}
                  />
                  ...
                  <div>
                    ...
                    {openingBalanceValue !== undefined && (
                      <p className="mt-1 text-[10px] text-[var(--theme-muted)]">
                        Opened at{' '}
                        {formatMoney(openingBalanceValue, accountCurrency)}
                        {openingBalanceDateValue &&
                          ` on ${openingBalanceDateValue}`}
                      </p>
                    )}
