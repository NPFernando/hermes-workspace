...
function buildGuardianContext(): GuardianContext {
  ...
  let openUnrealizedPnlQuote = 0
  
  // Get balance from finance store if available
  const usdtAccount = db.finance_accounts.find((acc: { currency: string; balance: number }) => acc.currency === 'USDT')
  if (usdtAccount) {
    ...
    handlers: {
      POST: async ({ request }) => {
        ...
        try {
          ...
          const quoteAmount = typeof body.quoteAmount === 'number' && body.quoteAmount > 0 ? body.quoteAmount : undefined
          ...
          if (!strategyId) {
            return json({ ok: false, error: 'strategyId is required' }, { status: 400 })
          }
          
          if (!quoteAmount) {
            return json({ ok: false, error: 'quoteAmount is required and must be positive' }, { status: 400 })
          }
          
          // Build the proposal and context
          const proposal = buildOrderProposal({ symbol, strategyId, quoteAmount })
          const context = buildGuardianContext()
          
          // Optional: allow override of guardian config
          const configOverride = body.config as Partial<GuardianConfig> | undefined
          const config = configOverride ? { ...DEFAULT_GUARDIAN_CONFIG, ...configOverride } : DEFAULT_GUARDIAN_CONFIG
