...
export function detectNovusOpportunity(taskContext: {
  ...
  if (
    unsuitableTypes.some((unsuitable) => normalizedType.includes(unsuitable))
  ) {
    return null
  }
  ...
  const isSuitableType = suitableTypes.some(
    (suitable) =>
  ...
  if (!isSuitableType) {
    ...
    if (description) {
      ...
      if (
        summaryKeywords.some((k) => descLower.includes(k)) ||
  ...
  if (
    ...
    normalizedType.includes('structured') ||
  ...
  const potentialSavingsPercent = Math.round(
    baseSavings *
