...
before anything renders, on every route. Route-level code splitting _works_
...
1. **Lazy-mount `ChatPanel` on non-chat routes** (`workspace-shell.tsx` imports it
   ...
   _other_ route currently pays 259 KB of chat screen plus the markdown pipeline
   (parse5, micromark, shiki core ≈ 230 KB more) at first paint.
   _Estimated: −300–450 KB from main. Risk: low-medium (Suspense fallback for the
   side panel; chat route itself unaffected)._
...
2. **Dedup animation libraries.** `motion` (v12, `motion-dom`) and `framer-motion`
   ...
   _Estimated: −30–90 KB. Risk: low (grep + swap imports, visual QA on animations)._
...
4. **Lazy the deferred-need components:** `settings-dialog` (open-on-click),
   ...
   _Estimated: −150–200 KB combined. Risk: low (each is an isolated `lazy()` +
   `Suspense` at an interaction boundary)._
